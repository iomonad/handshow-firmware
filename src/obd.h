...
static void pinMode(int iPin, int iMode) {} /* pinMode() */
...
const uint8_t ucBigFont[] PROGMEM = {
    ...
    0x00, 0x00, 0xc0, 0xc0, 0xc0, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x0f, 0x0f,
    ...
    0x03, 0x03, 0x0f, 0x0f, 0x3c, 0x3c, 0x00, 0x00, 0xf0, 0xf0, 0xc3, 0xc3,
    0x03, 0x03, 0x03, 0x03, 0x03, 0x03, 0xff, 0xff, 0xfc, 0xfc, 0x00, 0x00,
    0x00, 0x00, 0x03, 0x03, 0x03, 0x03, 0x3f, 0x3f, 0x3f, 0x3f, 0x03, 0x03,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x30, 0x3f, 0x3f, 0x0f, 0x0f,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    ...
    0xc0, 0xc0, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0c, 0x0c, 0x0f, 0x0f,
    0x03, 0x03, 0x00, 0x00, 0x03, 0x03, 0x0f, 0x0f, 0x0c, 0x0c, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static void _I2CWrite(OBDISP *pOBD, unsigned char *pData, int iLen) {
  ...
  {
    if (pOBD->iDCPin != 0xff) {
      digitalWrite(pOBD->iDCPin,
                   (pData[0] == 0) ? LOW : HIGH); // data versus command
...
void obdWriteCommand(OBDISP *pOBD, unsigned char c) {} /* obdWriteCommand() */
...
void oledWriteFlashBlock(OBDISP *pOBD, uint8_t *s, int iLen) {
  ...
  while (((pOBD->iScreenOffset & iWidthMask) + iLen) >=
         pOBD->width) // if it will hit the page end
  {
    j = pOBD->width -
    ...
    obdSetPosition(pOBD, pOBD->iScreenOffset & iWidthMask,
                   (pOBD->iScreenOffset >> iWidthShift), 1);
...
int obdScrollBuffer(OBDISP *pOBD, int iStartCol, int iEndCol, int iStartRow,
  ...
  if (iStartCol < 0 || iStartCol >= pOBD->width || iEndCol < 0 ||
      iEndCol > pOBD->width || iStartCol > iEndCol) // invalid
    return -1;
  if (iStartRow < 0 || iStartRow >= (pOBD->height / 8) || iEndRow < 0 ||
      iEndRow >= (pOBD->height / 8) || iStartRow > iEndRow)
    return -1;
  iPitch = pOBD->width;
  if (bUp) {
    for (row = iStartRow; row <= iEndRow; row++) {
      ...
      for (col = iStartCol; col <= iEndCol; col++) {
        ...
        if (row < iEndRow)
          b |= (s[iPitch]
                << 7); // capture pixel of row below, except for last row
        *s++ = b;
      } // for col
    }   // for row
  }     // up
...
void obdSetPosition(OBDISP *pOBD, int x, int y, int bRender) {
  ...
  if (!bRender)
    return; // don't send the commands to the OLED if we're not rendering the
            // graphics now
...
void obdRepeatByte(OBDISP *pOBD, uint8_t b, int iLen) {
  ...
  int iWidthMask = pOBD->width - 1;
  int iWidthShift =
      (pOBD->width == 128) ? 7 : 6; // assume 128 or 64 pixels wide
  int iSizeMask = ((pOBD->width * pOBD->height) / 8) - 1;
  uint8_t ucTemp[128];
...
uint8_t *obdPlayAnimFrame(OBDISP *pOBD, uint8_t *pAnimation, uint8_t *pCurrent,
  ...
  int iBufferSize =
      (pOBD->width * pOBD->height) / 8; // size in bytes of the display devce
...
void obdDrawSprite(OBDISP *pOBD, uint8_t *pSprite, int cx, int cy, int iPitch,
  ...
  if (x + cx < 0 || y + cy < 0 || x >= pOBD->width || y >= pOBD->height ||
      pOBD->ucScreen == NULL)
...
void obdDrawTile(OBDISP *pOBD, const uint8_t *pTile, int x, int y,
  ...
  memset(ucTemp, 0,
         sizeof(ucTemp)); // we only set white pixels, so start from black
...
int obdSetPixel(OBDISP *pOBD, int x, int y, unsigned char ucColor,
                int bRender) {
  int i;
...
int obdScaledString(OBDISP *pOBD, int x, int y, char *szMsg, int iSize,
  ...
  if (iXScale == 0 || iYScale == 0 || szMsg == NULL || pOBD == NULL ||
      pOBD->ucScreen == NULL || x < 0 || y < 0 || x >= pOBD->width - 1 ||
      y >= pOBD->height - 1)
  ...
  while (*szMsg) {
    ...
    // we can't directly use the pointer to FLASH memory, so copy to a local
    // buffer
...
//
int obdWriteString(OBDISP *pOBD, int iScroll, int x, int y, char *szMsg,
  ...
  if (iSize == FONT_8x8) // 8x8 font
  {
    ...
    while (pOBD->iCursorX < pOBD->width && szMsg[i] != 0 &&
           pOBD->iCursorY < pOBD->height / 8) {
      if (iScroll < 8) // only display visible characters
      {
        ...
        //             obdWriteDataBlock(pOBD, &ucTemp[iFontSkip], iLen,
        //             bRender); // write character pattern
        pOBD->iCursorX += iLen;
        if (pOBD->iCursorX >= pOBD->width - 7 &&
            pOBD->wrap) // word wrap enabled?
        {
    ...
    while (pOBD->iCursorX < pOBD->width &&
           pOBD->iCursorY < (pOBD->height / 8) - 3 && szMsg[i] != 0) {
      if (iScroll < 16) // if characters are visible
      {
        ...
        pOBD->iCursorX += iLen;
        if (pOBD->iCursorX >= pOBD->width - 15 &&
            pOBD->wrap) // word wrap enabled?
        {
          pOBD->iCursorX = 0; // start at the beginning of the next line
          pOBD->iCursorY += 4;
  ...
  else if (iSize == FONT_16x16) // 8x8 stretched to 16x16
  {
    ...
    iFontSkip = iScroll & 15; // number of columns to initially skip
    while (pOBD->iCursorX < pOBD->width &&
           pOBD->iCursorY < (pOBD->height / 8) - 1 && szMsg[i] != 0) {
  ...
  else if (iSize == FONT_12x16) // 6x8 stretched to 12x16
  {
    ...
    while (pOBD->iCursorX < pOBD->width &&
      ...
      if (iScroll < 12) // if characters are visible
      {
        ...
        pOBD->iCursorX += iLen;
        if (pOBD->iCursorX >= pOBD->width - 11 &&
            pOBD->wrap) // word wrap enabled?
        {
          pOBD->iCursorX = 0; // start at the beginning of the next line
          pOBD->iCursorY += 2;
    ...
    iFontSkip = iScroll % 6;
    while (pOBD->iCursorX < pOBD->width &&
           pOBD->iCursorY < (pOBD->height / 8) && szMsg[i] != 0) {
...
void obdGetStringBox(GFXfont *pFont, char *szMsg, int *width, int *top,
  ...
  if (width == NULL || top == NULL || bottom == NULL || pFont == NULL ||
      szMsg == NULL)
    return; // bad pointers
  miny = 100;
  maxy = 0;
      ...
      for (tx = 0; tx < pGlyph->width; tx++) {
        if (uc == 0) {                          // need to read more font data
          ...
          if (tx >= pGlyph->width) {
            ...
            ucMask = 1 << (ty & 7); // destination bit number for this line
            ucClr = (ucColor) ? ucMask : 0;
            d = &pOBD->ucScreen[(ty >> 3) * iPitch +
                                dx]; // internal buffer dest
          }
...
//
int obdDrawGFX(OBDISP *pOBD, uint8_t *pBuffer, int iSrcCol, int iSrcRow,
               int iDestCol, int iDestRow, int iWidth, int iHeight,
               int iSrcPitch) {
  int y;

  if (iSrcCol < 0 || iSrcCol >= pOBD->width || iSrcRow < 0 ||
      iSrcRow > (pOBD->height / 8) - 1 || iDestCol < 0 ||
      iDestCol >= pOBD->width || iDestRow < 0 ||
      iDestRow >= (pOBD->height >> 3) || iSrcPitch <= 0)
    return -1; // invalid
...
void obdFill(OBDISP *pOBD, unsigned char ucData, int bRender) {
  ...
  if (pOBD->type == LCD_VIRTUAL ||
      pOBD->type >= SHARP_144x168) // pure memory, handle it differently
  {
    if (pOBD->ucScreen)
...
void obdDrawLine(OBDISP *pOBD, int x1, int y1, int x2, int y2, uint8_t ucColor,
  ...
  if (x1 < 0 || x2 < 0 || y1 < 0 || y2 < 0 || x1 >= pOBD->width ||
      x2 >= pOBD->width || y1 >= pOBD->height || y2 >= pOBD->height)
    return;
  ...
  if (abs(dx) > abs(dy)) {
    ...
    p = pStart =
        &pOBD->ucScreen[x1 + ((y >> 3) *
                              iPitch)]; // point to current spot in back buffer
    ...
    for (x = x1; x1 <= x2; x1++) {
      ...
      if (error < 0) {
        ...
        if (yinc > 0)
          ...
          mask >>= 1;
        if (mask ==
            0) // we've moved outside the current row, write the data we changed
        {
          obdSetPosition(pOBD, x, y >> 3, bRender);
          obdWriteDataBlock(pOBD, pStart, (int)(p - pStart),
                            bRender); // write the row we changed
...
//
// For drawing ellipses, a circle is drawn and the x and y pixels are scaled by
// a 16-bit integer fraction This function draws a single pixel and scales its
// position based on the x/y fraction of the ellipse
//
static void DrawScaledPixel(OBDISP *pOBD, int iCX, int iCY, int x, int y,
...
//
static void DrawScaledLine(OBDISP *pOBD, int iCX, int iCY, int x, int y,
                           int32_t iXFrac, int32_t iYFrac, uint8_t ucColor) {
  int iLen, x2;
  uint8_t *d, ucMask;
  int iPitch;
...
//
static void BresenhamCircle(OBDISP *pOBD, int iCX, int iCY, int x, int y,
                            int32_t iXFrac, int32_t iYFrac, uint8_t ucColor,
                            uint8_t bFill) {
...
void obdEllipse(OBDISP *pOBD, int iCenterX, int iCenterY, int32_t iRadiusX,
  ...
  y = iRadius;
  while (x <= y) {
    BresenhamCircle(pOBD, iCenterX, iCenterY, x, y, iXFrac, iYFrac, ucColor,
                    bFilled);
    x++;
    if (iDelta < 0) {
      iDelta += (4 * x) + 6;
...
void obdRectangle(OBDISP *pOBD, int x1, int y1, int x2, int y2, uint8_t ucColor,
  ...
  if (x1 < 0 || y1 < 0 || x2 < 0 || y2 < 0 || x1 >= pOBD->width ||
      y1 >= pOBD->height || x2 >= pOBD->width || y2 >= pOBD->height)
...
// VERTICAL_BYTES) to the requested form returns 0 for success, -1 for error
  ...
  else    // Horizontal bytes here
  {
    if (iFlags & OBD_ROTATE_90) {
      ...
      for (y = yStart; y != yEnd; y += dy, xDst++) {
        ...
        ucDstMask =
            (iFlags & OBD_LSB_FIRST) ? (1 << (xDst & 7)) : (0x80 >> (xDst & 7));
        for (x = xStart; x != xEnd; x += dx, yDst++) {
          if (s[x] & ucSrcMask) // set pixel, copy to dest
