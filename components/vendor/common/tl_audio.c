...
static const signed char idxtbl[] = {-1, -1, -1, -1, 2, 4, 6, 8,
                                     -1, -1, -1, -1, 2, 4, 6, 8};
...
#if (UI_AUDIO_ENABLE &&                                                        \
     CLOCK_SYS_CLOCK_HZ == 24000000) // only for ble master 24m system clock
_attribute_ram_code_
#endif
    void
    adpcm_to_pcm(signed short *ps, signed short *pd, int len) {
  int i;
...
#define TL_NOISE_SUPRESSION_ENABLE                                             \
  0 // TODO : too much calculation can have packet drop
...
void voice_iir(signed short *ps, signed short *pd, int *coef, int nsample,
               u8 shift) {
  int i = 0;
  int s = 0;
  for (i = 0; i < nsample; i++) {
...
void mic_encoder_data_read_ok(void) { buffer_mic_pkt_rptr++; }
...
int sdm_decode_ready(int nbyte_to_decode) {
  ...
  u16 num =
      ((buffer_sdm_wptr >> 1) - sdm_rptr) & ((TL_SDM_BUFFER_SIZE >> 2) - 1);
  return ((nbyte_to_decode >> 2) + num) < (TL_SDM_BUFFER_SIZE >> 2);
}

int sdm_word_in_buffer() {
  u16 num =
      ((buffer_sdm_wptr >> 1) - reg_aud_rptr) & ((TL_SDM_BUFFER_SIZE >> 2) - 1);
  return num;
}
...
int sdm_decode_data(int *ps, int nbyte) { return 0; }
