...
typedef enum { P256R1, ED25519 } ecc_curve_t;

typedef enum { SHA256, SHA512 } hash_alg_t;
 ...
 *
 * @param[in]   p_sk   Private key. Pointer must be aligned to a 4-byte
 * boundary.
 ...
 * @retval     -2:  Unaligned pointer provided.
 */
int mi_crypto_ecc_pubkey_compute(ecc_curve_t curve, const ecc256_sk_t sk,
                                 ecc256_pk_t pk);
 ...
 * @param[out]  p_ss   Shared secret. Pointer must be aligned to a 4-byte
 * boundary.
 *
 ...
 * @retval     -2:  Unaligned pointer provided.
 */
int mi_crypto_ecc_shared_secret_compute(ecc_curve_t curve, const ecc256_pk_t pk,
                                        ecc256_ss_t ss);
 ...
 * @retval     -2:   Unaligned pointer provided.
 */
int mi_crypto_ecc_sign(ecc_curve_t curve, const uint8_t *p_hash,
                       uint8_t *p_sig);
 ...
 * @retval     -2:  Unaligned pointer provided.
 */
int mi_crypto_ecc_verify(ecc_curve_t curve, const ecc256_pk_t pk,
                         const uint8_t *p_hash, const uint8_t *p_sig);

int mi_crypto_get_crt_len(uint16_t *p_dev_len, uint16_t *p_manu_len,
                          uint16_t *p_root_len);
...
int mi_crypto_hash_finish(void *p_ctx, uint8_t *p_digest,
                          uint8_t *p_digest_len);

int mi_crypto_sha256(uint8_t *p_in, uint32_t in_len, uint8_t *p_out);

int mi_crypto_hkdf_sha256(const uint8_t *key, uint8_t key_len,
                          const uint8_t *salt, uint8_t salt_len,
                          const uint8_t *info, uint16_t info_len, uint8_t *out,
                          uint8_t out_len);
...
int mi_crypto_hmac_finish(void *p_ctx, uint8_t *p_digest,
                          uint8_t *p_digest_len);

int mi_crypto_hmac_sha256(const uint8_t *k, uint32_t key_len,
                          const uint8_t *p_in, uint32_t in_len, uint8_t *p_out);

int mi_crypto_ccm_encrypt(const uint8_t *key, const uint8_t *iv, size_t iv_len,
                          const uint8_t *add, size_t add_len, const uint8_t *in,
                          size_t in_len, uint8_t *out, uint8_t *tag,
                          size_t tag_len);

int mi_crypto_ccm_decrypt(const uint8_t *key, const uint8_t *iv, size_t iv_len,
                          const uint8_t *add, size_t add_len, const uint8_t *in,
                          size_t in_len, uint8_t *out, const uint8_t *tag,
                          size_t tag_len);

int mi_crypto_record_write(uint8_t record_id, const uint8_t *p_data,
                           uint8_t len);
