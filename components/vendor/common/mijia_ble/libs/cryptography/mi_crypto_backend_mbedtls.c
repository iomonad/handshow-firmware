...
static int x509_check_time(const msc_crt_time *before,
                           const msc_crt_time *after) {
 ...
 */
static int x509_get_version(unsigned char **p, const unsigned char *end,
                            int *ver) {
  int ret;
  size_t len;
...
static int x509_get_name(unsigned char **p, const unsigned char *end,
  ...
  while (1) {
     ...
     */
    if ((ret = mbedtls_asn1_get_tag(
             p, end, &set_len, MBEDTLS_ASN1_CONSTRUCTED | MBEDTLS_ASN1_SET)) !=
        0)
      return (-91 + ret);
...
int mbedtls_crt_parse_der(const unsigned char *crt, size_t crt_sz,
   ...
   */
  int ver;
  if ((ret = x509_get_version(&p, end, &ver)) != 0 || (ver < 0 || ver > 2)) {
    ret = -4;
    goto done;
  }
  ...
  else /* if the bignum less than 32 bytes, prepend 0x00(the bignum is big
          endian) */
