...
#define MSC_XFER(CMD, INPUT, INPUT_L, OUTPUT, OUTPUT_L)                        \
  (msc_cb_t) {                                                                 \
    .cmd = CMD, .p_para = INPUT, .para_len = INPUT_L, .p_data = OUTPUT,        \
    .data_len = OUTPUT_L                                                       \
  }
...
static int msc_encode_twi_buf(msc_cb_t *p_cb, uint16_t *p_tx_len,
                              uint16_t *p_rx_len) {
#if (HAVE_MSC == MJA1)
  ...
  case MSC_ROOT_CERT_LEN:
    *p_tx_len = mjac_cmd_build_read(twi_buf, MJAC_SZ_CMD_MAX,
                                    MJAC_ZONE_ROOT_CRT, 2, p_cb->data_len);
    *p_rx_len = p_cb->data_len;
    break;

  case MSC_MANU_CERT_LEN:
    *p_tx_len = mjac_cmd_build_read(twi_buf, MJAC_SZ_CMD_MAX,
                                    MJAC_ZONE_MANU_CRT, 2, p_cb->data_len);
    *p_rx_len = p_cb->data_len;
    break;

  case MSC_DEV_CERT_LEN:
    *p_tx_len = mjac_cmd_build_read(twi_buf, MJAC_SZ_CMD_MAX, MJAC_ZONE_DEV_CRT,
                                    2, p_cb->data_len);
    *p_rx_len = p_cb->data_len;
    break;
  ...
  case MSC_MANU_CERT:
    *p_tx_len = mjac_cmd_build_read(twi_buf, MJAC_SZ_CMD_MAX,
                                    MJAC_ZONE_MANU_CRT, 0, p_cb->data_len);
    *p_rx_len = p_cb->data_len;
    break;

  case MSC_ROOT_CERT:
    *p_tx_len = mjac_cmd_build_read(twi_buf, MJAC_SZ_CMD_MAX,
                                    MJAC_ZONE_ROOT_CRT, 0, p_cb->data_len);
    *p_rx_len = p_cb->data_len;
    break;

  case MSC_ID:
    *p_tx_len = mjac_cmd_build_read(twi_buf, MJAC_SZ_CMD_MAX, MJAC_ZONE_PRODUCT,
                                    2, p_cb->data_len);
    *p_rx_len = p_cb->data_len;
    break;
  ...
  case MSC_SIGN:
    ...
    *p_tx_len = mjac_cmd_build_generate_signature(twi_buf, MJAC_SZ_CMD_MAX,
                                                  hash_buf, MJAC_SZ_HASH);
  ...
  case MSC_VERIFY:
    ...
    *p_tx_len = mjac_cmd_build_verify_signature(
        twi_buf, MJAC_SZ_CMD_MAX, pub_buf, MJAC_SZ_PUBKEY, sig_buf,
        MJAC_SZ_SIGN, hash_buf, MJAC_SZ_HASH);
    *p_rx_len = 1;
    break;
  ...
  case MSC_ECDHE:
    ...
    pub_buf[36] = 0x20;
    memcpy(pub_buf + 37, p_cb->p_para + 32, 32);
    *p_tx_len = mjac_cmd_build_establish_key(twi_buf, MJAC_SZ_CMD_MAX, pub_buf,
                                             MJAC_SZ_PUBKEY);
  ...
  case MSC_WR_MKPK:
    *p_tx_len = mjac_cmd_build_update(twi_buf, MJAC_SZ_CMD_MAX, MJAC_ZONE_USER,
                                      (p_cb->p_para[0]) * 512,
                                      &(p_cb->p_para[1]), (p_cb->para_len) - 1);
    *p_rx_len = 0;
    break;

  case MSC_RD_MKPK:
    *p_tx_len = mjac_cmd_build_read(twi_buf, MJAC_SZ_CMD_MAX, MJAC_ZONE_USER,
                                    (p_cb->p_para[0]) * 512, p_cb->data_len);
    *p_rx_len = p_cb->data_len;
    break;

  case MSC_ERASE:
    *p_tx_len = mjac_cmd_build_update(twi_buf, MJAC_SZ_CMD_MAX, MJAC_ZONE_USER,
                                      p_cb->p_para[0] * 512, buf, 500);
    *p_rx_len = 0;
    break;
...
#else
  PT_WAIT_UNTIL(&pt_msc_thd, mible_iic_scl_pin_read(m_p_iic_config->scl_port,
                                                    m_p_iic_config->scl_pin));
  m_iic_xfer_ack = 0;
...
int mi_msc_config(void *pwr_manage, const iic_config_t *p_iic_config,
                  pstimer_t *p_timer) {
...
int msc_get_cert_len(uint16_t *p_dev_len, uint16_t *p_manu_len,
                     uint16_t *p_root_len) {
#if (HAVE_MSC == MJA1)
  int ret;
  PT_BEGIN(&pt_mjac);
  ...
  if (p_dev_len != NULL) {
    msc_control_block =
        MSC_XFER(MSC_DEV_CERT_LEN, NULL, 0, (void *)p_dev_len, 2);
  ...
  if (p_manu_len != NULL) {
    msc_control_block =
        MSC_XFER(MSC_MANU_CERT_LEN, NULL, 0, (void *)p_manu_len, 2);
    PT_WAIT_THREAD(&pt_mjac, ret = msc_thread(&msc_control_block));
    if (ret == PT_ENDED) {
  ...
  if (p_root_len != NULL) {
    msc_control_block =
        MSC_XFER(MSC_ROOT_CERT_LEN, NULL, 0, (void *)p_root_len, 2);
    PT_WAIT_THREAD(&pt_mjac, ret = msc_thread(&msc_control_block));
    if (ret == PT_ENDED) {
  ...
  msc_control_block = MSC_XFER(MSC_CERTS_LEN, NULL, 0, (void *)&m_certs_len,
                               sizeof(m_certs_len));
...
int msc_ecdh(const uint8_t *p_curve, const uint8_t *p_le_pk,
             uint8_t *p_shared) {
...
int msc_ecdsa_sign(const uint8_t *p_curve, const uint8_t *p_hash,
                   uint8_t *p_sig) {
