...
void micro_ecc_init(void *p_ctx) { uECC_set_rng(ecc_rng); }
...
int micro_ecc_public_key_compute(void *p_ctx, uint8_t const *p_sk,
                                 uint8_t *p_pk) {
  ecc256_pk_t le_pk;
  ecc256_sk_t le_sk;
  const struct uECC_Curve_t *p_curve;
...
int micro_ecc_shared_secret_compute(void *p_ctx, uint8_t const *p_sk,
                                    uint8_t const *p_pk, uint8_t *p_ss) {
  ecc256_sk_t le_sk;
  ecc256_pk_t le_pk;
  ecc256_ss_t le_ss;
  const struct uECC_Curve_t *p_curve;
...
int micro_ecc_sign(void *p_ctx, uint8_t const *p_sk, uint8_t const *p_hash,
                   uint8_t *p_sig) {
  const struct uECC_Curve_t *p_curve;
  ...
  int ret = uECC_sign((const uint8_t *)le_sk, (const uint8_t *)le_hash, 32,
                      (uint8_t *)le_sig, p_curve);
  if (!ret) {
    return -3;
  }
...
int micro_ecc_verify(void *p_ctx, uint8_t const *p_pk, uint8_t const *p_hash,
                     uint8_t const *p_sig) {
  const struct uECC_Curve_t *p_curve;
  ecc256_pk_t le_pk;
  uint8_t le_hash[32];
  ...
  int ret = uECC_verify((const uint8_t *)le_pk, (const uint8_t *)le_hash, 32,
                        (uint8_t *)le_sig, p_curve);
  if (!ret) {
    return -3;
  }
