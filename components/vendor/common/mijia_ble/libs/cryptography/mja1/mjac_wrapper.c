...
#define CMD_VALIDATE(CMD, CMD_SZ, DATA_SZ)                                     \
  (CMD && CMD_SZ >= DATA_SZ + MJAC_SZ_HEAD_TAIL)
#else
#define LV_VALIDATE(L, V, MIN, MAX) 1
#define CMD_VALIDATE(CMD, CMD_SZ, DATA_SZ) 1
#endif

#define CMD_BUILD_HEAD(OUT, SZ, CODE) OUT[SZ++] = CODE & 0x1F
...
static const UINT8 ecc_curve_nist_p256[] = {0x2A, 0X86, 0X48, 0XCE,
                                            0X3D, 0X03, 0X01, 0X07};

UINT16 mjac_cmd_build_echo(UINT8 *cmd, UINT16 cmd_sz, const UINT8 *echo_data,
                           UINT16 echo_data_sz) {
  UINT16 sz = 0;
  UINT16 crc = 0;
...
UINT16 mjac_cmd_build_generate_random(UINT8 *cmd, UINT16 cmd_sz,
                                      UINT8 random_sz) {
  UINT16 sz = 0;
  UINT16 crc = 0;
...
UINT16 mjac_cmd_build_hibernate(UINT8 *cmd, UINT16 cmd_sz) {
  UINT16 sz = 0;
  UINT16 crc = 0;
...
UINT16 mjac_cmd_build_read(UINT8 *cmd, UINT16 cmd_sz, mjac_zone_t idx,
...
UINT16 mjac_cmd_build_generate_signature(UINT8 *cmd, UINT16 cmd_sz,
...
UINT16 mjac_cmd_build_verify_signature(UINT8 *cmd, UINT16 cmd_sz,
                                       const UINT8 *pubkey, UINT16 pubkey_sz,
                                       const UINT8 *signature,
                                       UINT16 signature_sz, const UINT8 *hash,
                                       UINT16 hash_sz) {
  UINT16 sz = 0;
  UINT16 crc = 0;
  ...
  if (!LV_VALIDATE(pubkey_sz, pubkey, MJAC_SZ_PUBKEY, MJAC_SZ_PUBKEY) ||
      ...
      !CMD_VALIDATE(cmd, cmd_sz,
                    1 + sizeof(ecc_curve_nist_p256) + pubkey_sz + signature_sz +
                        hash_sz))
    return 0;
...
UINT16 mjac_cmd_build_establish_key(UINT8 *cmd, UINT16 cmd_sz,
                                    const UINT8 *pubkey, UINT16 pubkey_sz) {
  UINT16 sz = 0;
  UINT16 crc = 0;
...
UINT16 mjac_cmd_build_generate_key(UINT8 *cmd, UINT16 cmd_sz) {
  UINT16 sz = 0;
  UINT16 crc = 0;
...
UINT16 mjac_cmd_build_query(UINT8 *cmd, UINT16 cmd_sz) {
  UINT16 sz = 0;
  UINT16 crc = 0;
...
mjac_resp_code_t mjac_resp_parse_data(UINT8 *resp, UINT16 *data_sz) {
...
UINT16 mjac_cmd_build_update(UINT8 *cmd, UINT16 cmd_sz, mjac_zone_t idx,
                             UINT16 offset, const UINT8 *data, UINT16 data_sz) {
  UINT16 sz = 0;
  UINT16 crc = 0;

  if (idx != MJAC_ZONE_USER || offset >= 4096 || !(data && data_sz) ||
      !(cmd && cmd_sz >= 4 + data_sz)) {
