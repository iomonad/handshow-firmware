...
static bool verify_firmware(mible_dfu_tag_t *tag, void *root_cert,
                            uint16_t root_len, void *cert, void *sign,
                            uint16_t sign_len);
...
mible_status_t mible_dfu_auth(uint32_t total_size, uint32_t product_id) {
  ...
  ret = mible_dfu_flash_read(&tag_size, sizeof(tag_size),
                             offset + 16); /* 16 bytes magic */
  if (MI_SUCCESS != ret) {
  ...
  sign_size = total_size - p_dfu_tag->firmware_size - tag_size -
              p_dfu_tag->certification_size;
...
static bool verify_firmware(mible_dfu_tag_t *tag, void *root_cert,
  ...
  int ret;
  uint8_t child_sig[64], child_sha[32],
  ...
  cert_len = mbedtls_crt_pem2der(cert, tag->certification_size, buffer_for_cert,
  ...
  cert_len = mbedtls_crt_pem2der(cert_develop + strlen(PEM_FOOTER),
                                 tag->certification_size, buffer_for_cert,
                                 MIBLE_DFU_CERT_MAX_SIZE);
  if (0 == cert_len) {
