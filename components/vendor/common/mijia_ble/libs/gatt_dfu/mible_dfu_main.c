...
static mible_status_t mible_dfu_send_evt(uint8_t event, uint8_t *param,
                                         uint16_t len);
...
mible_status_t mible_dfu_init(mible_dfu_init_t *p_init) {
  ...
  ret = queue_init(&dfu_config.evt_queue, evt_buffer, MIBLE_DFU_EVT_BUF_MAX,
                   MIBLE_DFU_EVT_SIZE_MAX);
  if (MI_SUCCESS != ret) {
...
mible_status_t mible_dfu_ctrl(uint8_t *buffer, uint16_t size) {
  ...
  if (NULL == p_cmd || size < offsetof(mible_dfu_cmd_t, param) ||
  ...
  case MIBLE_DFU_COMMAND_READ_VERSION: {
    ...
    ret =
        mible_dfu_send_evt(MIBLE_DFU_EVENT_CMD_COMPLETE, param, sizeof(param));
    break;
  }
...
static mible_status_t mible_dfu_send_evt(uint8_t event, uint8_t *param,
  ...
  if (NULL == param ||
      len > MIBLE_DFU_EVT_SIZE_MAX - offsetof(mible_dfu_evt_t, param)) {
    return MI_ERR_INVALID_PARAM;
  }
...
static void mible_dfu_timer_callback(void *arg) {
  ...
  if (0 == PT_SCHEDULE(mible_dfu_sar_handler(&dfu_config.pt_sar_ctrl))) {
    if (dfu_fragment.index > 0 &&
        dfu_fragment.index >= dfu_config.current_index) {
...
static int mible_dfu_fragment_handler(pt_t *pt) {
  ...
  PT_WAIT_UNTIL(pt, (MI_SUCCESS ==
                     mible_dfu_flash_write(
                         dfu_fragment.data, MIBLE_DFU_FRAGMENT_SIZE,
                         (dfu_fragment.index - 1) * MIBLE_DFU_FRAGMENT_SIZE)) ||
                        ((--retry_num) <= 0));

  /* Read the copy of fragment to RAM */
  PT_WAIT_UNTIL(
      pt, (MI_SUCCESS == mible_dfu_flash_read(buffer, MIBLE_DFU_FRAGMENT_SIZE,
                                              (dfu_fragment.index - 1) *
                                                  MIBLE_DFU_FRAGMENT_SIZE)) ||
              ((--retry_num) <= 0));
  ...
  if (0 == memcmp(dfu_fragment.data, buffer, MIBLE_DFU_FRAGMENT_SIZE)) {
    ...
    if (1 == dfu_fragment.index) {
      dfu_config.total_size =
          dfu_config.current_size - sizeof(dfu_fragment.index);
...
static int mible_dfu_sar_handler(pt_t *pt) {
  ...
  PT_SPAWN(pt, &dfu_config.pt_rx_ctrl,
           rxfer_rx_thd(&dfu_config.pt_rx_ctrl, &dfu_config.rxfer_dfu,
                        PASS_THROUGH, &dfu_config.current_size));
  ...
  if (MI_SUCCESS == dequeue(&dfu_config.evt_queue, tmp)) {
    ...
    if (MI_SUCCESS ==
        mible_gatts_notify_or_indicate(
            dfu_config.init.conn_handle, dfu_config.init.srv_handle,
            dfu_config.init.ctrl_handle, 0, tmp,
            p_evt->length + offsetof(mible_dfu_evt_t, param), 1)) {
