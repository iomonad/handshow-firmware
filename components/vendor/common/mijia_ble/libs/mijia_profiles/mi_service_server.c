...
typedef struct {
  ...
  uint16_t
      conn_handle; /**< Handle of the current connection (as provided by the BLE
                      Stack). BLE_CONN_HANDLE_INVALID if not in a connection. */
  uint16_t service_handle; /**< Handle of Xiaomi Service (as provided by the BLE
                              Stack). */

  uint16_t version_handle; /**< Handle related to the characteristic value (as
                              provided by the BLE Stack). */
  uint16_t ctrl_point_handle; /**< Handle related to the characteristic value
                                 (as provided by the BLE Stack). */
  uint16_t auth_handle; /**< Handle related to the characteristic value (as
                           provided by the BLE Stack). */
...
#define BLE_UUID_MI_VERS                                                       \
  0x0004 /**< The UUID of the Mi Service Version Characteristic. */
#define BLE_UUID_MI_CTRLP                                                      \
  0x0010 /**< The UUID of the Control Point Characteristic. */
#define BLE_UUID_MI_SECURE                                                     \
  0x0016 /**< The UUID of the Secure Auth Characteristic. */
#define BLE_UUID_MI_OTA_CTRL                                                   \
  0x0017 /**< The UUID of the OTA Control Point Characteristic. */
#define BLE_UUID_MI_OTA_DATA                                                   \
  0x0018 /**< The UUID of the OTA Data Characteristic. */
#define BLE_UUID_MI_STANDARD                                                   \
  0x0019 /**< The UUID of the Standard Auth Characteristic. */
...
static pstimer_t rxfer_timer = {.p_curr_tick = &schd_ticks,
                                .tick_interval_ms = 10};
...
static void on_connect(mible_gap_evt_param_t *p_ble_evt) {
  ...
  if (p_ble_evt->connect.conn_param.max_conn_interval >
      prefer_params.max_conn_interval) {
...
/**@brief Function for handling the @ref BLE_GAP_EVT_DISCONNECTED event from the
 * BLE Stack.
 *
 * @param[in] p_mi_s    Xiaomi Service structure.
 * @param[in] p_ble_evt Pointer to the event received from BLE stack.
...
/**@brief Function for handling the @ref BLE_GAP_EVT_CONN_PARAM_UPDATE event
 * from the BLE Stack.
 *
 * @param[in] p_mi_s    Xiaomi Service structure.
 * @param[in] p_ble_evt Pointer to the event received from BLE stack.
...
static void on_conn_params_update(mible_gap_evt_param_t *p_ble_evt) {
  MI_LOG_INFO("update interval max: %d ms, min: %d ms\n",
              ...
              p_ble_evt->update_conn.conn_param.max_conn_interval * 1250 /
                  1000);
}

/**@brief Function for handling the @ref BLE_GATTS_EVT_WRITE event from the BLE
 * Stack.
 *
 * @param[in] p_mi_s    Xiaomi Service structure.
 * @param[in] p_ble_evt Pointer to the event received from BLE stack.
...
static void on_write_permit(mible_gatts_evt_param_t *p_ble_evt) {
  ...
  if (handle == mi_srv.ota_ctrl_point) {
    if (get_mi_authorization() == ADMIN_AUTHORIZATION) {
      mible_gatts_rw_auth_reply(mi_srv.conn_handle, 1, handle, 0, pdata, len,
                                2);
...
*/
    mible_gatts_value_set(mi_srv.service_handle, mi_srv.version_handle, 0,
...
static void arch_event_handler(mible_arch_event_t evt,
  ...
  case MIBLE_ARCH_EVT_GATTS_SRV_INIT_CMP:
    if (param->srv_init_cmp.p_gatts_db->p_srv_db->srv_uuid.uuid16 ==
        BLE_UUID_MI_SERVICE)
...
uint32_t mi_service_init() {
  ...
  MI_LOG_DEBUG("mijia libs\t : %d.%d.%d\n", MIBLE_LIB_MAJOR, MIBLE_LIB_MINOR,
               MIBLE_LIB_REVISION);
  ...
  // mible_gatts_srv_db_t *p_mi_service = (mible_gatts_srv_db_t*) malloc(
  // sizeof(mible_gatts_srv_db_t) ); mible_gatts_char_db_t *p_mi_char =
  // (mible_gatts_char_db_t *) malloc( MI_SECURE_CHAR_NUM *
  // sizeof(mible_gatts_char_db_t) );
  mible_gatts_srv_db_t *p_mi_service =
      (mible_gatts_srv_db_t *)(&mi_gatts_srv_db);
  mible_gatts_char_db_t *p_mi_char =
      (mible_gatts_char_db_t *)(mi_gatt_char_tbl);
  ...
  *p_mi_service = (mible_gatts_srv_db_t){
  ...
  p_mi_char[CHAR_OTA_CP_IDX] =
      (mible_gatts_char_db_t){// .char_uuid = (mible_uuid_t){.type = 0, .uuid16
                              // = BLE_UUID_MI_OTA_CTRL, },
...
uint32_t opcode_send(uint32_t status) {
  ...
  errno = mible_gatts_notify_or_indicate(
      mi_srv.conn_handle, mi_srv.service_handle, mi_srv.ctrl_point_handle, 0,
      (void *)&status, sizeof(status), 1);
...
uint32_t opcode_recv(void) { return m_opcode; }
