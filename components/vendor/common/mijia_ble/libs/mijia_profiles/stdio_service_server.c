...
#define BLE_UUID_STDIO_SRV                                                     \
  {                                                                            \
    0x6D, 0x69, 0x2E, 0x6D, 0x69, 0x6F, 0x74, 0x2E, 0x62, 0x6C, 0x65, 0x00,    \
        0x00, 0x01, 0x00, 0x00                                                 \
  }
#define BLE_UUID_STDIO_RX                                                      \
  {                                                                            \
    0x6D, 0x69, 0x2E, 0x6D, 0x69, 0x6F, 0x74, 0x2E, 0x62, 0x6C, 0x65, 0x00,    \
        0x01, 0x01, 0x00, 0x00                                                 \
  }
#define BLE_UUID_STDIO_TX                                                      \
  {                                                                            \
    0x6D, 0x69, 0x2E, 0x6D, 0x69, 0x6F, 0x74, 0x2E, 0x62, 0x6C, 0x65, 0x00,    \
        0x02, 0x01, 0x00, 0x00                                                 \
  }
...
typedef struct {
  ...
  uint16_t rx_handle; /**< Handle related to the characteristic value (as
                         provided by the BLE Stack). */
  uint16_t tx_handle; /**< Handle related to the characteristic value (as
                         provided by the BLE Stack). */
...
int stdio_service_init(stdio_rx_t rx_handler) {
  ...
  mible_gatts_srv_db_t *p_lock_service =
      (mible_gatts_srv_db_t *)&lock_studio_service;
  mible_gatts_char_db_t *p_lock_char =
      (mible_gatts_char_db_t *)lock_studio_char;
...
int stdio_tx(void *data, uint8_t len) {
  ...
  errno = mible_gatts_notify_or_indicate(
      m_stdio.conn_handle, m_stdio.service_handle, m_stdio.tx_handle, 0, tx_buf,
      tx_len, 1);
  if (errno != MI_SUCCESS)
    return -3;
