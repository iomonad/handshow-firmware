 ...
 *
 * @details The Lock Service is a simple GATT-based service with many
 * characteristics. Data received from the peer is passed to the application,
 * and the data received from the application of this service is sent to the
 * peer as Handle Value Notifications. This module demonstrates how to implement
 * a custom GATT-based service and characteristics using the BLE Stack. The
 * service is used by the application to send and receive pub_key and MSC Cert
 * to and from the peer.
 *
 * @note The application must propagate BLE Stack events to the Lock Service
 * module by calling the ble_mi_on_ble_evt() function from the ble_stack_handler
 * callback.
 */
 ...
 * service. The application must fill this structure and pass it to the service
 * using the @ref ble_mi_init function.
 */
typedef struct {
  lock_handler_t opcode_handler; /**< Event handler to be called for handling
                                    received data. */
} lock_init_t;
 ...
 * @retval NRF_ERROR_NULL If either of the pointers p_mi_s or p_mi_s_init is
 * NULL.
 */
 ...
 *                    2: bolted
 *
 * @details This function MUST be called in 5 seconds after the lock opcode
 * received. Otherwise, Mijia App will disconnect it.
 *
