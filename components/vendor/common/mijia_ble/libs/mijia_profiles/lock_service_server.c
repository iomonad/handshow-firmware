...
#define BLE_UUID_LOCK_SRV                                                      \
  {                                                                            \
    0x6D, 0x69, 0x2E, 0x6D, 0x69, 0x6F, 0x74, 0x2E, 0x62, 0x6C, 0x65, 0x00,    \
        0x00, 0x10, 0x00, 0x00                                                 \
  }

#define BLE_UUID_LOCK_OPS                                                      \
  {                                                                            \
    0x6D, 0x69, 0x2E, 0x6D, 0x69, 0x6F, 0x74, 0x2E, 0x62, 0x6C, 0x65, 0x00,    \
        0x01, 0x10, 0x00, 0x00                                                 \
  }
#define BLE_UUID_LOCK_STATE                                                    \
  {                                                                            \
    0x6D, 0x69, 0x2E, 0x6D, 0x69, 0x6F, 0x74, 0x2E, 0x62, 0x6C, 0x65, 0x00,    \
        0x02, 0x10, 0x00, 0x00                                                 \
  }
#define BLE_UUID_LOCK_LOGS                                                     \
  {                                                                            \
    0x6D, 0x69, 0x2E, 0x6D, 0x69, 0x6F, 0x74, 0x2E, 0x62, 0x6C, 0x65, 0x00,    \
        0x03, 0x10, 0x00, 0x00                                                 \
  }
...
typedef struct {
  ...
  uint16_t service_handle; /**< Handle of Lock Service (as provided by the BLE
                              Stack). */

  uint16_t operation_handle; /**< Handle related to the characteristic value (as
                                provided by the BLE Stack). */
  uint16_t state_handle;     /**< Handle related to the characteristic value (as
                                provided by the BLE Stack). */
  uint16_t logs_handle;      /**< Handle related to the characteristic value (as
                                provided by the BLE Stack). */
...
/**@brief Function for handling the @ref BLE_GATTS_EVT_READ event from the BLE
 * Stack.
 *
 * @param[in] p_mi_s    Lock Service structure.
 * @param[in] p_ble_evt Pointer to the event received from BLE stack.
...
static void on_read(mible_gatts_evt_param_t *p_ble_evt) {
  ...
  if (handle == m_lock_srv.state_handle) {
    if (get_mi_authorization() == UNAUTHORIZATION) {
      mible_gatts_rw_auth_reply(m_lock_srv.conn_handle, 0, handle, 0, NULL, 0,
                                1);
    } else {
      mible_gatts_rw_auth_reply(m_lock_srv.conn_handle, 1, handle, 0,
                                lock_state, sizeof(lock_state), 1);
    }
    ...
    if (get_mi_authorization() == UNAUTHORIZATION) {
      ...
      mible_gatts_rw_auth_reply(m_lock_srv.conn_handle, 1, handle, 0, lock_logs,
                                sizeof(lock_logs), 1);
    }
  } else {
...
/**@brief Function for handling the @ref BLE_GATTS_EVT_WRITE_PERMIT event from
 * the BLE Stack.
 *
 * @param[in] p_mi_s    Lock Service structure.
 * @param[in] p_ble_evt Pointer to the event received from BLE stack.
...
static void on_write_permit(mible_gatts_evt_param_t *p_ble_evt) {
  ...
  if (handle == m_lock_srv.operation_handle) {
    ...
    if (get_mi_authorization() == UNAUTHORIZATION) {
      ...
      mible_gatts_rw_auth_reply(m_lock_srv.conn_handle, 1, handle, 0, pdata,
                                len, 2);
      uint8_t opcode = 0;
...
static void service_init_succ(mible_arch_gatts_srv_init_cmp_t *param) {
  ...
  if (param->status == MI_SUCCESS) {
    ...
    m_lock_srv.operation_handle =
        p_char_db[CHAR_OPERATION_IDX].char_value_handle;
    ...
    MI_LOG_INFO(
        "char value handle list\n operation : %d\n state : %d\n logs : %d\n",
        m_lock_srv.operation_handle, m_lock_srv.state_handle,
        m_lock_srv.logs_handle);
  }
...
int lock_service_init(lock_init_t *p_config) {
  ...
  mible_gatts_srv_db_t *p_lock_service =
      (mible_gatts_srv_db_t *)&lock_service_server;
  mible_gatts_char_db_t *p_lock_char =
      (mible_gatts_char_db_t *)lock_char_server;
  ...
  *p_lock_service = (mible_gatts_srv_db_t){
  ...
  p_lock_char[CHAR_OPERATION_IDX] =
      (mible_gatts_char_db_t){.char_uuid = uuid,
...
int reply_lock_stat(uint8_t status) {
  ...
  errno = mible_gatts_notify_or_indicate(
      m_lock_srv.conn_handle, 0, m_lock_srv.state_handle, 0, tx_buf, tx_len, 1);
  if (errno != MI_SUCCESS)
    return -3;
...
int send_lock_log(uint16_t type, uint8_t vlen, void *value) {
  ...
  errno = mible_gatts_notify_or_indicate(
      m_lock_srv.conn_handle, 0, m_lock_srv.logs_handle, 0, tx_buf, tx_len, 1);
  if (errno != MI_SUCCESS)
    return -3;
