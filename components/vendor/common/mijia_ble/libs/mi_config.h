...
#define CONCAT_VERSION(x, y, z, u)                                             \
  STR_VAL(x) "." STR_VAL(y) "." STR_VAL(z) "_" STR_VAL(u)
...
#else
#error                                                                         \
    "No MI_BLE_ENABLED or MI_MESH_ENABLED is defined. Should add one of them in the preprocesser symbols."
#endif

#define MIBLE_LIB_AND_DEVELOPER_VERSION                                        \
  CONCAT_VERSION(MIBLE_LIB_MAJOR, MIBLE_LIB_MINOR, MIBLE_LIB_REVISION,         \
                 DEVELOPER_VERSION)
 ...
 *
 * The mibeacon object is an adv message contains the status or event. BLE
 * gateway can receive the beacon message (by active scanning) and upload it to
 * server for triggering customized home automation scene.
 *
 ...
 *       OOB_USE_NUMPAD_INPUT  (bit 0): device has a numeric keypad to enter the
 * pairing code. OOB_USE_QR_CODE_OUT   (bit 4): device provided with a QR code
 * label. OOB_USE_DISPLAT_OUT   (bit 7): device displayed a six digit number.
 */
...
#define OOB_IO_CAP                                                             \
  (OOB_USE_NUMPAD_INPUT | (OOB_USE_DISPLAT_OUT) << 4 |                         \
   (OOB_USE_QR_CODE_OUT) << 7)
