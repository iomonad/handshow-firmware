 ...
 *          [in] stop_adv: When the object queue is sent out, it will SHUTDOWN
 * BLE advertising
 *
 * @return  MI_SUCCESS             Successfully enqueued a object into the
 * object queue. MI_ERR_DATA_SIZE       Object value length is too long.
 *          MI_ERR_RESOURCES       Object queue is full. Please try again later.
 *          MI_ERR_INTERNAL        Can not invoke the sending handler.
 *
 * @note    This function ONLY works when the device has been registered and has
 * restored the keys.
 *
 * The mibeacon object is an adv message contains the status or event. BLE
 * gateway can receive the beacon message (by BLE scanning) and upload it to
