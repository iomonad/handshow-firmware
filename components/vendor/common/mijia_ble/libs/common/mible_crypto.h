 ...
 *
 * @details After Secure auth login, device and application will both generate
 * the same session key. This key will used to encrypt the data by AES128-CCM.
 * The output is concatenating LSB of counter in nonce, cipher text and the MIC.
 * So the length of output buffer MUST be 6 bytes larger than input buffer.
 *
