...
static void mibeacon_timer_handler(void *p_context) {
  ...
  if (dequeue(&mi_obj_queue, &obj) == MI_ERR_NOT_FOUND) {
    ...
    errno = mible_gap_adv_data_set(adv_data, adv_dlen, scan_rsp_data,
                                   scan_rsp_dlen);
    MI_ERR_CHECK(errno);
 ...
 *          [out] p_output: pointer to mibeacon data  (watch out array out of
 * bounds) [out] p_output_len: pointer to mibeacon data length
 * @return  MI_ERR_INVALID_PARAM:   Invalid pointer supplied or mismatched
 * frmctl. MI_ERR_INVALID_LENGTH:  Adv data length exceeds
 * MIBLE_MAX_ADV_LENGTH-7. MI_ERR_INTERNAL:        Not found rand num used to
 * encrypt data. MI_SUCCESS:             Set successfully.
 * */
static mible_status_t mibeacon_encode(mibeacon_config_t const *const config,
                                      uint8_t *output, uint8_t *output_len) {
  ...
  *output++ = (uint8_t)seqnum.value;
  ...
  if (config->p_obj != NULL) {
    ...
    uint8_t mic[4];
    uint8_t aad = 0x11;
    errno = mi_crypto_ccm_encrypt(beacon_key, (uint8_t *)&beacon_nonce,
                                  sizeof(beacon_nonce), &aad, sizeof(aad),
                                  objs_ptr, objs_len, objs_ptr, mic, 4);
    MI_ERR_CHECK(errno);
    if (errno)
...
static mible_status_t adv_start(uint16_t interval_ms, uint8_t is_connected) {
  ...
  mible_gap_adv_param_t param = {
      ...
      .adv_type = is_connected ? MIBLE_ADV_TYPE_NON_CONNECTABLE_UNDIRECTED
                               : MIBLE_ADV_TYPE_CONNECTABLE_UNDIRECTED,
  };
  if (interval_ms > 500) {
...
mible_status_t mibeacon_init(uint8_t key[16], uint8_t is_newone) {
  queue_init(&mi_obj_queue, (void *)obj_buf, ARRAY_SIZE(obj_buf),
             sizeof(obj_buf[0]));

  mi_service_event_register(mi_service_handler);

  if (mibeacon_timer == NULL)
    mible_timer_create(&mibeacon_timer, mibeacon_timer_handler,
...
mible_status_t mibeacon_adv_stop(void) { return mible_gap_adv_stop(); }
