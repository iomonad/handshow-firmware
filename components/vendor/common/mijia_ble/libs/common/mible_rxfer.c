...
_attribute_ram_code_ static uint8_t
rxfer_rx_decode(rxfer_cb_t *pxfer, uint8_t *pdata, uint8_t len) {
...
static int rxfer_tx_cmd(rxfer_cb_t *pxfer, fctrl_cmd_t cmd, ...) {
  ...
  errno =
      mible_gatts_notify_or_indicate(pxfer->conn_handle, 0, pxfer->value_handle,
...
static int rxfer_rx_ack(rxfer_cb_t *pxfer, fctrl_ack_t ack, ...) {
  ...
  frame.ctrl.type = ack;
  data_len =
      sizeof(frame.sn) + sizeof(frame.ctrl.type) + sizeof(frame.ctrl.mode);
...
void rxfer_process(rxfer_cb_t *pxfer, uint8_t *pdata, uint16_t len) {
  ...
  if (curr_sn == FRAME_CTRL) {
    ...
    } else if (pxfer->state == RXFER_WAIT_ACK &&
...
_attribute_ram_code_ int rxfer_rx_thd(pt_t *pt, rxfer_cb_t *pxfer,
                                      uint8_t data_type, ...) {
  static uint16_t sn;
  PT_BEGIN(pt);
  ...
  if (data_type == SERVER_CERT || data_type == PASS_THROUGH ||
...
_attribute_ram_code_ int rxfer_tx_thd(pt_t *pt, rxfer_cb_t *pxfer,
                                      uint8_t data_type) {
  static uint16_t sn;
  PT_BEGIN(pt);

  /* Send data */
  PT_WAIT_UNTIL(pt, rxfer_tx_cmd(pxfer, (fctrl_cmd_t)data_type,
                                 pxfer->tx_num) == MI_SUCCESS);
