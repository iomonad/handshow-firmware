...
int mi_session_init(session_ctx_t *p_ctx) {
  ...
  uint8_t errno = mible_timer_create(&session_timer, session_timeout,
...
int mi_session_encrypt(const uint8_t *input, uint8_t ilen, uint8_t *output) {
  ...
  ret = mi_crypto_ccm_encrypt(session_ctx.dev_key, (void *)&nonce,
                              sizeof(nonce), NULL, 0, tmp, ilen, 2 + output,
                              2 + output + ilen, 4);
...
int mi_session_decrypt(const uint8_t *input, uint8_t ilen, uint8_t *output) {
  ...
  ret = mi_crypto_ccm_decrypt(session_ctx.app_key, (void *)&nonce,
                              sizeof(nonce), NULL, 0, 2 + input, ilen - 2 - 4,
                              output, 2 + input + ilen - 2 - 4, 4);
