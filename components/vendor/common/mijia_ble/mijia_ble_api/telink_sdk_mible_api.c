...
u8 telink_ble_mi_att_sync(u8 att_op, mible_gatts_srv_db_t *p) {
  ...
  mible_gatts_srv_db_t *p_gatts_db_ini =
      (evt_param_t.srv_init_cmp.p_gatts_db->p_srv_db);
  if (att_op == 0) {
...
u8 gap_adv_param_valid(mible_gap_adv_param_t *p_param) {
  ...
  if (p_param->adv_interval_min > p_param->adv_interval_max) {
    valid = 0;
  } else if (p_param->adv_interval_min < 20 ||
             p_param->adv_interval_min > 0x4000) {
    valid = 0;
  } else if (p_param->adv_interval_max < 20 ||
             p_param->adv_interval_max > 0x4000) {
    valid = 0;
...
mible_status_t telink_ble_mi_adv_start(mible_gap_adv_param_t *p_param) {
  ...
  status = bls_ll_setAdvParam(
      p_param->adv_interval_min, p_param->adv_interval_max, p_param->adv_type,
      OWN_ADDRESS_PUBLIC, 0, NULL, chn_mask, ADV_FP_NONE);
...
u8 mi_update_conn_params_valid(mible_gap_conn_param_t conn_params) {
  ...
  if (conn_params.max_conn_interval < conn_params.min_conn_interval) {
    valid = 0;
  } else if (conn_params.max_conn_interval > 0x0C80 ||
             conn_params.max_conn_interval < 0x0006) {
    valid = 0;
  } else if (conn_params.min_conn_interval > 0x0C80 ||
             conn_params.min_conn_interval < 0x0006) {
    valid = 0;
  } else if (conn_params.slave_latency > 0x01F3 ||
             conn_params.slave_latency < 0) {
    valid = 0;
  } else if (conn_params.conn_sup_timeout > 0x0C80 ||
             conn_params.conn_sup_timeout < 0x000A) {
    valid = 0;
  }
  return valid;
}
...
// user function ,and will call by the mi part
mible_status_t
telink_ble_mi_update_conn_params(uint16_t conn_handle,
...
mible_status_t telink_ble_mi_gatts_service_init(mible_gatts_db_t *p_server_db) {
  ...
  handle = find_handle_by_uuid_service(p_uuid_srv_str->type,
                                       p_uuid_srv_str->uuid128, my_Attributes);
  if (handle == 0) {
    return MI_ERR_INVALID_PARAM;
  }
  ...
  for (u8 i = 0; i < (p_srv->char_num); i++) {
    ...
    handle = find_handle_by_uuid_char(p_uuid_char_str->type,
                                      p_uuid_char_str->uuid128, my_Attributes);
...
mible_status_t telink_ble_mi_gatts_value_set(uint16_t srv_handle,
...
mible_status_t telink_ble_mi_gatts_value_get(uint16_t srv_handle,
                                             uint16_t value_handle,
...
mible_status_t telink_ble_mi_gatts_notify_or_indicate(
...
void telink_record_part_init() {
  ...
  while (1) {
    ...
    if (telink_record.rec_id == 0xffff ||
        record_adr >= FLASH_ADR_MI_RECORD_TMP) {
...
u8 telink_record_clean_cpy() {
  ...
  while (1) { // cpy the record to tmp first
    flash_read_page(record_adr, sizeof(telink_record_t), p_buf);
    if (telink_record.rec_id == 0xffff ||
        record_adr >= FLASH_ADR_MI_RECORD_MAX) {
      break;
  ...
  while (cpy_cnt) {
    if (cpy_cnt > sizeof(telink_record_t)) {
      flash_read_page(FLASH_ADR_MI_RECORD_TMP + idx, sizeof(telink_record_t),
                      p_buf);
      flash_write_page(FLASH_ADR_MI_RECORD + idx, sizeof(telink_record_t),
                       p_buf);
...
mible_status_t telink_record_create(uint16_t record_id, uint8_t len) {
  /*
          ...
          u32 record_adr =0;
          if(flash_idx_adr + (len+3)+RECORD_RESERVE_SPACE >
     FLASH_ADR_MI_RECORD_TMP){
...
mible_status_t telink_record_delete(uint16_t record_id) {
  ...
  if (!find_record_adr(record_id, &record_adr)) {
    telink_record_eve_cb(record_id, MI_ERR_INVALID_PARAM,
...
mible_status_t telink_record_read(uint16_t record_id, uint8_t *p_data,
                                  uint8_t len) {
  u32 record_adr = 0;
  uint8_t *p_buf = (u8 *)(&telink_record);
  if (len > RECORD_MAX_LEN || len == 0) {
  ...
  if (len <= sizeof(telink_record.dat)) {
    ...
    // should dispatch whether all the data is zero ,if it's means it have not
    // write already
    /*
    if(buf_is_empty_or_not(p_data,len)){
            return MI_ERR_INVALID_PARAM;
...
void telink_record_eve_cb(uint16_t record_id, mible_status_t sts,
                          mible_arch_event_t eve) {
...
mible_status_t telink_record_write(uint16_t record_id, uint8_t *p_data,
                                   uint8_t len) {
  uint8_t total_len = len;
  uint8_t buf_idx = 0;
  uint8_t *p_buf = (u8 *)(&telink_record);
  if (len > RECORD_MAX_LEN || len == 0) {
    telink_record_eve_cb(record_id, MI_ERR_INVALID_LENGTH,
...
/*
 * @brief   Post a task to a task quene, which can be executed in a right
 * place(maybe a task in RTOS or while(1) in the main function).
...
mible_status_t telink_mi_task_post(mible_handler_t handler, void *arg) {
  ...
  if (my_fifo_push(&mi_task_api_func_t, (u8 *)(&task_fun),
                   sizeof(mi_task_fun_t)) == 0) {
    return MI_ERR_NO_MEM;
  } else {
    return MI_SUCCESS;
  }
...
void mi_event_beacon_proc() {
  ...
  if (clock_time_exceed(mi_event_beacon_tick, 60 * 1000 * 1000)) { // 1 minute
    ...
    if (mi_event_min % battery_inter ==
        0) { // send power every 59 min,to avoid the battery check
...
static void advertising_init(bool solicite_bind) {
  ...
  if (MI_SUCCESS !=
      mible_service_data_set(&mibeacon_cfg, adv_data + 3, &service_data_len)) {
...
u8 mi_get_encrypt_busy_flag() { return mi_ceti_busy_flag; }
