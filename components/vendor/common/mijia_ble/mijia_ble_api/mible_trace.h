...
/* DWT (Data Watchpoint and Trace) registers, only exists on ARM Cortex-M3 above
 * with a DWT unit */
#ifdef DWT
...
#define InitCycleCounter() CoreDebug->DEMCR |= TRCENA_BIT

#define ResetCycleCounter() DWT->CYCCNT = 0

#define EnableCycleCounter() DWT->CTRL |= DWT_CYCCNTENA_BIT

#define DisableCycleCounter() DWT->CTRL &= ~DWT_CYCCNTENA_BIT
