...
typedef struct {
  uint16_t scan_interval; // Range: 0x0004 to 0x4000 Time = N * 0.625 msec Time
                          // Range: 2.5 msec to 10.24 sec
  uint16_t scan_window;   // Range: 0x0004 to 0x4000 Time = N * 0.625 msec Time
                          // Range: 2.5 msec to 10.24 seconds
  uint16_t timeout; // Scan timeout between 0x0001 and 0xFFFF in seconds, 0x0000
                    // disables timeout.
...
typedef struct {
  uint16_t adv_interval_min; // Range: 0x0020 to 0x4000  Time = N * 0.625 msec
                             // Time Range: 20 ms to 10.24 sec
  uint16_t adv_interval_max; // Range: 0x0020 to 0x4000  Time = N * 0.625 msec
                             // Time Range: 20 ms to 10.24 sec
  mible_gap_adv_type_t adv_type;

  struct {
    uint8_t ch_37_off : 1; /**< Setting this bit to 1 will turn off advertising
                              on channel 37 */
    uint8_t ch_38_off : 1; /**< Setting this bit to 1 will turn off advertising
                              on channel 38 */
    uint8_t ch_39_off : 1; /**< Setting this bit to 1 will turn off advertising
                              on channel 39 */
  } ch_mask;
...
typedef struct {
  uint16_t min_conn_interval; // Range: 0x0006 to 0x0C80, Time = N * 1.25 msec,
                              // Time Range: 7.5 msec to 4 seconds.
  uint16_t max_conn_interval; // Range: 0x0006 to 0x0C80, Time = N * 1.25 msec,
                              // Time Range: 7.5 msec to 4 seconds.
  uint16_t slave_latency;     // Range: 0x0000 to 0x01F3
  uint16_t conn_sup_timeout;  // Range: 0x000A to 0x0C80, Time = N * 10 msec,
...
typedef enum {
  MIBLE_GAP_EVT_CONNECTED =
      MIBLE_GAP_EVT_BASE,   /**< Generated when a connection is established.*/
...
/*
 * NOTE: if char property contains notify , then SHOULD include cccd(client
 * characteristic configuration descriptor automatically). The same to sccd when
 * BROADCAST enabled
 * */
typedef struct {
  mible_gatts_char_desc_ext_prop_t *extend_prop;
  mible_gatts_char_desc_cpf_t *char_format; // See more details at Bluetooth
                                            // SPEC 4.2 [Vol 3, Part G] Page 539
...
typedef struct {
  ...
  bool rd_author; // read authorization. Enabel or Disable
                  // MIBLE_GATTS_READ_PERMIT_REQ event
  bool wr_author; // write authorization. Enabel or Disable
                  // MIBLE_GATTS_WRITE_PERMIT_REQ event
...
typedef enum {
  MIBLE_GATTS_EVT_WRITE =
      MIBLE_GATTS_EVT_BASE, // When this event is called, the characteristic has
                            // been modified.
  MIBLE_GATTS_EVT_READ_PERMIT_REQ,  // If charicteristic's rd_auth = TRUE, this
                                    // event will be generated.
  MIBLE_GATTS_EVT_WRITE_PERMIT_REQ, // If charicteristic's wr_auth = TRUE, this
                                    // event will be generated, meanwhile the
                                    // char value hasn't been modified.
                                    // mible_gatts_rw_auth_reply().
 ...
 * parameters
 * NOTE: Stack SHOULD decide whether to response to gatt client. And if need to
 * reply, just reply success or failure according to [permit]
 * */
typedef struct {
 ...
 * NOTE: Stack SHOULD decide to reply the char value or refuse according to
 * [permit]
 * */
typedef struct {
...
/*
 * MIBLE_GATTC_EVT_NOTIFICATION or MIBLE_GATTC_EVT_INDICATION event callback
 * parameters
 *  */
typedef struct {
  uint16_t handle;
