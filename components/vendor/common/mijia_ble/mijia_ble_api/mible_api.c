 ...
 *@note     You should support this function in corresponding asynchronous
 *function. For now, mible_gatts_service_int and mible_record_write is
 *asynchronous.
 * */
 ...
 * */
__WEAK mible_status_t mible_gap_scan_stop(void) { return MI_SUCCESS; }
 ...
 * @param   [in] p_data : Raw data to be placed in advertising packet. If NULL,
 * no changes are made to the current advertising packet.
 * @param   [in] dlen   : Data length for p_data. Max size: 31 octets. Should be
 * 0 if p_data is NULL, can be 0 if p_data is not NULL.
 * @param   [in] p_sr_data : Raw data to be placed in scan response packet. If
 * NULL, no changes are made to the current scan response packet data.
 * @param   [in] srdlen : Data length for p_sr_data. Max size:
 * BLE_GAP_ADV_MAX_SIZE octets. Should be 0 if p_sr_data is NULL, can be 0 if
 * p_data is not NULL.
 ...
 *          MI_ERR_INVALID_STATE   Initiated connection procedure in connected
 * state. MI_ERR_INVALID_PARAM   Invalid parameter(s) supplied. MI_ERR_BUSY The
 * stack is busy, process pending events and retry. MI_ERR_RESOURCES       Stop
 * one or more currently active roles (Central, Peripheral or Observer) and try
 * again MIBLE_ERR_GAP_INVALID_BLE_ADDR    Invalid Bluetooth address supplied.
 ...
 *role.
 * */
__WEAK mible_status_t mible_gap_update_conn_params(
    uint16_t conn_handle, mible_gap_conn_param_t conn_params) {
 ...
 * @note    This function can be implemented asynchronous. When service inition
 * complete, call mible_arch_event_callback function and pass in
 * MIBLE_ARCH_EVT_GATTS_SRV_INIT_CMP event and result.
 * */
...
__WEAK mible_status_t mible_gatts_value_set(uint16_t srv_handle,
                                            ...
                                            uint8_t len) {
  return telink_ble_mi_gatts_value_set(srv_handle, value_handle, offset,
                                       p_value, len);
}
...
__WEAK mible_status_t mible_gatts_value_get(uint16_t srv_handle,
  ...
  return telink_ble_mi_gatts_value_get(srv_handle, value_handle, p_value,
                                       p_len);
}
 ...
 * (notification or indication) has been enabled by the client.
 * */
__WEAK mible_status_t mible_gatts_notify_or_indicate(
    uint16_t conn_handle, uint16_t srv_handle, uint16_t char_value_handle,
    uint8_t offset, uint8_t *p_value, uint8_t len, uint8_t type) {
  return telink_ble_mi_gatts_notify_or_indicate(
      conn_handle, srv_handle, char_value_handle, offset, p_value, len, type);
}
 ...
 *          [in] offset: the offset from which the attribute value has to
 * be updated
 *          [in] p_value: Pointer to new value used to update the attribute
 * value. [in] len: data length [in] type : read response = 1; write response =
 * 2;
 *
 * @return  MI_SUCCESS             Successfully queued a response to the peer,
 * and in the case of a write operation, GATT updated. MI_ERR_INVALID_ADDR
 * Invalid pointer supplied. MI_ERR_INVALID_PARAM   Invalid parameter (offset)
 * supplied. MI_ERR_INVALID_STATE   Invalid Connection State or no authorization
 * request pending. MI_ERR_INVALID_LENGTH  Invalid length supplied. MI_ERR_BUSY
 * Procedure already in progress. MIBLE_ERR_ATT_INVALID_HANDLE     Attribute not
 * found.
 * @note    This call should only be used as a response to a
 * MIBLE_GATTS_EVT_READ/WRITE_PERMIT_REQ event issued to the application.
 * */
__WEAK mible_status_t mible_gatts_rw_auth_reply(
    uint16_t conn_handle, uint8_t status, uint16_t char_value_handle,
    uint8_t offset, uint8_t *p_value, uint8_t len, uint8_t type) {
  return MI_SUCCESS;
}
 ...
 * */
__WEAK mible_status_t mible_gattc_char_discover_by_uuid(
    uint16_t conn_handle, mible_handle_range_t handle_range,
 ...
 * */
__WEAK mible_status_t mible_gattc_read_char_value_by_uuid(
    uint16_t conn_handle, mible_handle_range_t handle_range,
    mible_uuid_t *p_char_uuid) {
  return MI_SUCCESS;
}
 ...
 * @param   [out] p_timer_id: a pointer to timer id address which can uniquely
 * identify the timer. [in] timeout_handler: a pointer to a function which can
 * be called when the timer expires. [in] mode: repeated or single shot.
 ...
 *          When record writing complete , call mible_arch_event_callback
 * function and pass MIBLE_ARCH_EVT_RECORD_WRITE_CMP event and result.
 * */
__WEAK mible_status_t mible_record_write(uint16_t record_id,
 ...
 * */
__WEAK void mible_tasks_exec(void) { telink_mi_task_exec(); }
 ...
 *          after the transfer has completed successfully (allowing for a
 * repeated start in the next transfer).
 * @return  MI_SUCCESS              The command was accepted.
 ...
 *          When tx procedure complete, the handler provided by mible_iic_init()
 * should be called, and the iic event should be passed as a argument.
 * */
 ...
 * @note    This function should be implemented in non-blocking mode.
 *          When rx procedure complete, the handler provided by mible_iic_init()
 * should be called, and the iic event should be passed as a argument.
 * */
 ...
 *          0: Low (Busy)
 * */
__WEAK int mible_iic_scl_pin_read(uint8_t port, uint8_t pin) { return 0; }
