...
typedef struct ev_loop_ctrl_t {
  ev_event_t *events[1]; /* Registered file events and callback */
                         /*
                                 There could be dozens of registered events, may be only several fired.
                                 Loop a small fired array is more efficient than that loop all then
                            check fired-flag.
                         */
  ...
  int fired_index;
  /*
          Pool callbacks can be added and removed frequently. We could use
     double-linked list instead. But it may be less than several vaiable poll
     events, use array is faster.
  */
  ev_poll_t poll[1];
  /*
          Time events is sorted, use single linked list
...
#endif
enum {
  EV_TIMER_SAFE_MARGIN_US = 40000000,
  EV_TIMER_SAFE_MARGIN = (EV_TIMER_SAFE_MARGIN_US * CLOCK_SYS_CLOCK_1US)
}; // in us,
