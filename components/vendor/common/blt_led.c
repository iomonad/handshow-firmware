...
void device_led_init(
    u32 gpio, u8 polarity) { // polarity: 1 for high led on, 0 for low led on
  ...
  if (device_led.isOn) {
    if (clock_time_exceed(device_led.startTick, device_led.onTime_ms * 1000)) {
      ...
      if (device_led.offTime_ms) { // offTime not zero
        device_led.startTick +=
            device_led.onTime_ms * CLOCK_16M_SYS_TIMER_CLK_1MS;
      } else {
        device_led.repeatCount = 0;
      }
    }
    ...
    if (clock_time_exceed(device_led.startTick, device_led.offTime_ms * 1000)) {
      if (--device_led.repeatCount) {
        device_led_on_off(1);
        device_led.startTick +=
            device_led.offTime_ms * CLOCK_16M_SYS_TIMER_CLK_1MS;
      }
    }
  }
#endif
