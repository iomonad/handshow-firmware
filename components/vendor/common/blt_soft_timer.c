    ...
    for (int i = 0; i < n - 1; i++) {
      for (int j = 0; j < n - i - 1; j++) {
        if (TIME_COMPARE_BIG(blt_timer.timer[j].t, blt_timer.timer[j + 1].t)) {
          ...
          memcpy(&blt_timer.timer[j], &blt_timer.timer[j + 1],
                 sizeof(blt_time_event_t));
...
int blt_soft_timer_add(blt_timer_callback_t func, u32 interval_us) {
  ...
  if (blt_timer.currentNum >= MAX_TIMER_NUM) { // timer full
    ...
    blt_timer.timer[blt_timer.currentNum].interval =
        interval_us * CLOCK_16M_SYS_TIMER_CLK_1US;
    blt_timer.timer[blt_timer.currentNum].t =
        now + blt_timer.timer[blt_timer.currentNum].interval;
...
int blt_soft_timer_delete_by_index(u8 index) {
  if (index >= blt_timer.currentNum) {
    write_reg32(0x40000, 0x11111121);
    while (1)
      ; // debug ERR
    return 0;
  }
  ...
  for (int i = index; i < blt_timer.currentNum - 1; i++) {
    memcpy(&blt_timer.timer[i], &blt_timer.timer[i + 1],
           sizeof(blt_time_event_t));
  }
  ...
  for (int i = 0; i < blt_timer.currentNum; i++) {
    if (blt_timer.timer[i].cb == func) {
      ...
      if (i == 0) { // ɾ�����������timer����Ҫ����ʱ��
        ...
        if ((u32)(blt_timer.timer[0].t - clock_time()) <
            3000 * CLOCK_16M_SYS_TIMER_CLK_1MS) {
...
void blt_soft_timer_process(int type) {
  ...
  for (int i = 0; i < blt_timer.currentNum; i++) {
    if (blt_is_timer_expired(blt_timer.timer[i].t, now)) { // timer trigger
      ...
      if (blt_timer.timer[i].cb == NULL) {
        write_reg32(0x40000, 0x11111122);
        while (1)
          ; // debug ERR
      } else {
  ...
  if (blt_timer.currentNum) { // timer table not empty
    ...
    if ((u32)(blt_timer.timer[0].t - now) <
        3000 * CLOCK_16M_SYS_TIMER_CLK_1MS) {
      bls_pm_setAppWakeupLowPower(blt_timer.timer[0].t, 1);
    } else {
      bls_pm_setAppWakeupLowPower(0, 0); // disable
