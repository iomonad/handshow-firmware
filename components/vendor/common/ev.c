...
*/
void ev_enable_poll(ev_poll_e e) { loop->poll[e].valid = 1; }

void ev_disable_poll(ev_poll_e e) { loop->poll[e].valid = 0; }
...
void ev_clear_event(void) { loop->fired_count = 0; }

void ev_process_event() {
  int i;
  int fired_count =
      loop->fired_count; //  cache count, because p->cb will emit new events,
                         //  will change loop->fired_count
  ...
  for (i = loop->fired_index; i < end_index; ++i) {
    ...
    if (fe->e < TR_T_EVENT_E - TR_T_EVENT_0) {
      LOG_TICK((TR_T_EVENT_0 + fe->e),
               ev_call_callbacks(fe->e, fe->data)); // may emit new event
    } else
#endif
  ...
  u8 r = irq_disable();
  loop->fired_count -=
      fired_count; // loop->fired_count may bring up race condition, if
                   // ev_emit_event is called in irq_handler
  irq_restore(r);
  loop->fired_index =
      (loop->fired_index + fired_count) & EV_FIRED_EVENT_MAX_MASK;
}
...
static int ev_cmp_timer(ev_time_event_t *e1, ev_time_event_t *e2, u32 now) {
  return (int)(ev_cal_timer_distant(e1->t, now) -
               ev_cal_timer_distant(e2->t, now));
}
...
void ev_set_timer(ev_time_event_t *e, int t_us) {
  // assert(t_us < (CLOCK_MAX_US - EV_TIMER_SAFE_MARGIN_US));	// about 143000
  // in 30M clock, about 2 minutes
...
void ev_start_timer(ev_time_event_t *e) {
  ...
  if (0 == e->id) {
    ...
    log_data(TR_24_TIMERS_ADDR + e->id,
             (u32)e); // to show relation of id and the address
  }
#endif
  irq_restore(r);
}
  ...
  while (te) {
    if (ev_is_timer_expired(te, now)) {
      ...
      if (prev_head != loop->timer_head) {
        // restart the whole from timer_head.  because the head could be changed
        // within the loop
        te = loop->timer_head;
        prev_head = te;
      } else {
