...
// static assertion. evaluate at compile time. It is very useful like,
// STATIC_ASSERT(sizeof(a) == 5);

// #define STATIC_ASSERT(expr)   			{ char
// static_assertion[(expr) ? 1 : -1]; ((void) static_assertion); }	//
// (void) array;  to remove compiler unused variable warning

// more complicated version canbe used anywhere in the source
#define STATIC_ASSERT_M(COND, MSG)                                             \
...
#define STATIC_ASSERT_POW2(expr)                                               \
  STATIC_ASSERT(!((expr) & ((expr)-1))) //  assert  expr  is  2**N
