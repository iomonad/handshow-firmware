...
// RF_LEN_MAX:255: MIC(4)+LL_MAX_LEN(251) =>
// LL_MAX_LEN:l2cap_len(2)+cid(2)+ATT_MTU_MAX(247). l2cap buffer max:
// dma(4)+header(2)+l2cap_len(2)+cid(2)+ATT_MTU_MAX(247).
...
/******************************* User Interface
 * ************************************/
void bls_l2cap_requestConnParamUpdate(u16 min_interval, u16 max_interval,
                                      u16 latency, u16 timeout); // Slave
...
void blc_l2cap_SendConnParamUpdateResponse(u16 connHandle, u8 req_id,
                                           conn_para_up_rsp result);
