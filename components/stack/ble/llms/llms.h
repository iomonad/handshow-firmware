...
/********************* Macro & Enumeration & Structure Definition for Stack,
 * user can not use!!!!  **********************/
...
#define BLMS_FLG_RF_CONN_DONE                                                  \
  (FLD_RF_IRQ_CMD_DONE | FLD_RF_IRQ_FIRST_TIMEOUT | FLD_RF_IRQ_RX_TIMEOUT |    \
   FLD_RF_IRQ_RX_CRC_2)
...
typedef struct {
  ...
  u8 conn_dma_tx_rptr;
  u8 smp_empty_pkt_entry; // slave-role empty pkt entry: ble main_loop entry
                          // enable flg
  u8 blt_tx_pkt_hold;
  u8 rsvd;
...
ble_sts_t blms_att_pushNotifyData(u16 connHandle, u16 attHandle, u8 *p,
                                  int len);
