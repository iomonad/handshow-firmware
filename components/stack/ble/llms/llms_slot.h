...
#define SLOT_TASK_MASTER                                                       \
  (SLOT_TASK_MASTER1 | SLOT_TASK_MASTER2 | SLOT_TASK_MASTER3)
...
// #define			SLOT_UPDT_MASTER_TERMINATE_ALL
// BIT(6)

// #define			SLOT_UPDT_ADV_DISBALE
// BIT(6) #define			SLOT_UPDT_ADV_ENABLE
// BIT(7)

#define SLOT_DURATION_SET_SCAN 1
#define SLOT_DURATION_ADV                                                      \
  3 // ADV use 3*1.25mS=3.75mS most(may change later, depend on actual test
    // value)
#define SLOT_DURATION_SLAVE_COMMON 3
#define SLOT_DURATION_MASTER_COMMON 2

#define PKT_INIT_AFTER_RX 550 // at least: 352+150
#define BOUNDARY_MARGIN_INIT                                                   \
  900 // PKT_INIT_AFTER_RX + slot_update_rebuild_allocate running potential
      // biggest time
...
typedef struct {
  ...
  slot_type slot_idx_irq_real; // used for timing calculate when initiate, see
                               // the explanations in blms_m_connect
  ...
  u32 slot_tick_irq_real; // used for timing calculate when initiate, see the
                          // explanations in blms_m_connect
...
static inline void blt_slot_increaseConnSlotPriority(
    u8 conn_idx) { // increase priority due to packet loss & previous slot drop
                   // ...
...
static inline void blt_slot_addTask(u8 task) { bltSlot.task_mask |= (task); }

static inline void blt_slot_removeTask(u8 task) {
  bltSlot.task_mask &= ~(task);
}

static inline void blt_slot_clearTask(void) { bltSlot.task_mask = 0; }

static inline void blt_slot_addUpdate(u8 updt) { bltSlot.update |= (updt); }

static inline void blt_slot_removeUpdate(u8 updt) { bltSlot.update &= ~(updt); }
