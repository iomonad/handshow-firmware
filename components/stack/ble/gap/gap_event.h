...
#define GAP_EVT_MASK_DEFAULT                                                   \
  (GAP_EVT_MASK_SMP_TK_DISPALY || GAP_EVT_MASK_SMP_TK_REQUEST_PASSKEY ||       \
   GAP_EVT_MASK_SMP_TK_REQUEST_OOB || GAP_EVT_MASK_ATT_EXCHANGE_MTU)
...
typedef struct {
  u16 connHandle;
  u8 re_connect; // 1: re_connect, encrypt with previous distributed LTK;   0:
                 // paring , encrypt with STK
