...
#define ATT_OP_FIND_INFO_RSP                                                   \
  0x05 //!< Find Information Response op code		----Information Data 4
       //!< to (ATT_MTU-2)
#define ATT_OP_FIND_BY_TYPE_VALUE_REQ                                          \
  0x06 //!< Find By Type Vaue Request op code			Attribute Value
       //!< 0 to (ATT_MTU-7)
#define ATT_OP_FIND_BY_TYPE_VALUE_RSP                                          \
  0x07 //!< Find By Type Vaue Response op code	----Handles Information List 4
       //!< to (ATT_MTU-1)
#define ATT_OP_READ_BY_TYPE_REQ 0x08 //!< Read By Type Request op code
#define ATT_OP_READ_BY_TYPE_RSP                                                \
  0x09                       //!< Read By Type Response op code			----Attribute
                             //!< Data List 2 to (ATT_MTU- 2)
#define ATT_OP_READ_REQ 0x0a //!< Read Request op code
#define ATT_OP_READ_RSP                                                        \
  0x0b                            //!< Read Response op code
                                  //!< @@--Attribute Value 0 to (ATT_MTU-1)
#define ATT_OP_READ_BLOB_REQ 0x0c //!< Read Blob Request op code
#define ATT_OP_READ_BLOB_RSP                                                   \
  0x0d //!< Read Blob Response op code			@@--Part Attribute Value
       //!< 0 to (ATT_MTU-1)
#define ATT_OP_READ_MULTI_REQ                                                  \
  0x0e //!< Read Multiple Request op code				Set Of
       //!< Handles 4 to (ATT_MTU-1)
#define ATT_OP_READ_MULTI_RSP                                                  \
  0x0f //!< Read Multiple Response op code		----Set Of Values 0 to
       //!< (ATT_MTU-1)
#define ATT_OP_READ_BY_GROUP_TYPE_REQ                                          \
  0x10 //!< Read By Group Type Request op code
#define ATT_OP_READ_BY_GROUP_TYPE_RSP                                          \
  0x11 //!< Read By Group Type Response op code	----Attribute Data List 4 to
       //!< (ATT_MTU- 2)
#define ATT_OP_WRITE_REQ                                                       \
  0x12                        //!< Write Request op code
                              //!< Attribute Value 0 to (ATT_MTU-3)
#define ATT_OP_WRITE_RSP 0x13 //!< Write Response op code
#define ATT_OP_PREPARE_WRITE_REQ                                               \
  0x16 //!< Prepare Write Request op code				Part
       //!< Attribute Value 0 to (ATT_MTU-5)
#define ATT_OP_PREPARE_WRITE_RSP                                               \
  0x17 //!< Prepare Write Response op code		----Part Attribute Value
       //!< 0 to (ATT_MTU-5)
...
#define ATT_OP_HANDLE_VALUE_NOTI                                               \
  0x1b //!< Handle Value Notification op code			Attribute Value
       //!< 0 to (ATT_MTU-3)
#define ATT_OP_HANDLE_VALUE_IND                                                \
  0x1d //!< Handle Value Indication op code			Attribute Value
       //!< 0 to (ATT_MTU-3)
#define ATT_OP_HANDLE_VALUE_CFM 0x1e //!< Handle Value Confirmation op code
#define ATT_OP_WRITE_CMD                                                       \
  0x52 //!< ATT Write Command
       //!< Attribute Value 0 to (ATT_MTU-3)
#define ATT_OP_SIGNED_WRITE_CMD                                                \
  0xd2 //!< ATT Signed Write Command
       //!< Attribute Value 0 to (ATT_MTU - 15)

/** @defgroup ATT_PERMISSIONS_BITMAPS GAP ATT Attribute Access Permissions Bit
 * Fields
 * @{
 * (See the Core_v5.0(Vol 3/Part C/10.3.1/Table 10.2) for more information)
 */
#define ATT_PERMISSIONS_AUTHOR                                                 \
  0x10 // Attribute access(Read & Write) requires Authorization
#define ATT_PERMISSIONS_ENCRYPT                                                \
  0x20 // Attribute access(Read & Write) requires Encryption
#define ATT_PERMISSIONS_AUTHEN                                                 \
  0x40 // Attribute access(Read & Write) requires Authentication(MITM
       // protection)
#define ATT_PERMISSIONS_SECURE_CONN                                            \
  0x80 // Attribute access(Read & Write) requires Secure_Connection
#define ATT_PERMISSIONS_SECURITY                                               \
  (ATT_PERMISSIONS_AUTHOR | ATT_PERMISSIONS_ENCRYPT | ATT_PERMISSIONS_AUTHEN | \
   ATT_PERMISSIONS_SECURE_CONN)
...
#define ATT_PERMISSIONS_RDWR                                                   \
  (ATT_PERMISSIONS_READ |                                                      \
   ATT_PERMISSIONS_WRITE) //!< Attribute is Readable & Writable

#define ATT_PERMISSIONS_ENCRYPT_READ                                           \
  (ATT_PERMISSIONS_READ | ATT_PERMISSIONS_ENCRYPT) //!< Read requires Encryption
#define ATT_PERMISSIONS_ENCRYPT_WRITE                                          \
  (ATT_PERMISSIONS_WRITE |                                                     \
   ATT_PERMISSIONS_ENCRYPT) //!< Write requires Encryption
#define ATT_PERMISSIONS_ENCRYPT_RDWR                                           \
  (ATT_PERMISSIONS_RDWR |                                                      \
   ATT_PERMISSIONS_ENCRYPT) //!< Read & Write requires Encryption

#define ATT_PERMISSIONS_AUTHEN_READ                                            \
  (ATT_PERMISSIONS_READ | ATT_PERMISSIONS_ENCRYPT |                            \
   ATT_PERMISSIONS_AUTHEN) //!< Read requires Authentication
#define ATT_PERMISSIONS_AUTHEN_WRITE                                           \
  (ATT_PERMISSIONS_WRITE | ATT_PERMISSIONS_ENCRYPT |                           \
   ATT_PERMISSIONS_AUTHEN) //!< Write requires Authentication
...
#define ATT_PERMISSIONS_SECURE_CONN_READ                                       \
  (ATT_PERMISSIONS_READ | ATT_PERMISSIONS_SECURE_CONN |                        \
   ATT_PERMISSIONS_ENCRYPT |                                                   \
   ATT_PERMISSIONS_AUTHEN) //!< Read requires Secure_Connection
#define ATT_PERMISSIONS_SECURE_CONN_WRITE                                      \
  (ATT_PERMISSIONS_WRITE | ATT_PERMISSIONS_SECURE_CONN |                       \
   ATT_PERMISSIONS_ENCRYPT |                                                   \
   ATT_PERMISSIONS_AUTHEN) //!< Write requires Secure_Connection
#define ATT_PERMISSIONS_SECURE_CONN_RDWR                                       \
  (ATT_PERMISSIONS_RDWR | ATT_PERMISSIONS_SECURE_CONN |                        \
   ATT_PERMISSIONS_ENCRYPT |                                                   \
   ATT_PERMISSIONS_AUTHEN) //!< Read & Write requires Secure_Connection

#define ATT_PERMISSIONS_AUTHOR_READ                                            \
  (ATT_PERMISSIONS_READ |                                                      \
   ATT_PERMISSIONS_AUTHOR) //!< Read requires Authorization
#define ATT_PERMISSIONS_AUTHOR_WRITE                                           \
  (ATT_PERMISSIONS_WRITE |                                                     \
   ATT_PERMISSIONS_AUTHEN) //!< Write requires Authorization
#define ATT_PERMISSIONS_AUTHOR_RDWR                                            \
  (ATT_PERMISSIONS_RDWR |                                                      \
   ATT_PERMISSIONS_AUTHOR) //!< Read & Write requires Authorization

/** @} End GAP_ATT_PERMISSIONS_BITMAPS */

// See the Core_v5.0(Vol 3/Part F/3.4.1.1, "Error Response") for more
// information.
typedef enum {

  ATT_SUCCESS = 0,

  ATT_ERR_INVALID_HANDLE, //!< The attribute handle given was not valid on this
                          //!< server
  ...
  ATT_ERR_INSUFFICIENT_AUTH,   //!< The attribute requires authentication before
                               //!< it can be read or written
  ATT_ERR_REQ_NOT_SUPPORTED, //!< Attribute server does not support the request
                             //!< received from the client
  ATT_ERR_INVALID_OFFSET,    //!< Offset specified was past the end of the
                             //!< attribute
  ATT_ERR_INSUFFICIENT_AUTHOR, //!< The attribute requires authorization before
                               //!< it can be read or written
  ATT_ERR_PREPARE_QUEUE_FULL,  //!< Too many prepare writes have been queued
  ATT_ERR_ATTR_NOT_FOUND, //!< No attribute found within the given attribute
                          //!< handle range
  ATT_ERR_ATTR_NOT_LONG,  //!< The attribute cannot be read or written using the
                          //!< Read Blob Request
  ATT_ERR_INSUFFICIENT_KEY_SIZE,  //!< The Encryption Key Size used for
                                  //!< encrypting this link is insufficient
  ATT_ERR_INVALID_ATTR_VALUE_LEN, //!< The attribute value length is invalid for
                                  //!< the operation
  ATT_ERR_UNLIKELY_ERR, //!< The attribute request that was requested has
                        //!< encountered an error that was unlikely, and
                        //!< therefore could not be completed as requested
  ATT_ERR_INSUFFICIENT_ENCRYPT, //!< The attribute requires encryption before it
                                //!< can be read or written
  ATT_ERR_UNSUPPORTED_GRP_TYPE, //!< The attribute type is not a supported
                                //!< grouping attribute as defined by a higher
                                //!< layer specification
  ATT_ERR_INSUFFICIENT_RESOURCES, //!< Insufficient Resources to complete the
                                  //!< request
...
typedef struct {
  ...
  u16 attHandleInErr; //!< The attribute handle that generated this error
                      //!< response
} errorRsp_t;
 ...
 */
typedef struct {
  u16 handle;  //!< The handle of the attribute to be written (must be first
               //!< field)
  u8 len;      //!< Length of value
 ...
 */
typedef struct {
  u8 flags; //!< 0x00 - cancel all prepared writes 0x01 - immediately write all
            //!< pending prepared values
} executeWriteReq_t;
 ...
 * @{
 */
#define CHAR_PROP_BROADCAST                                                    \
  0x01                      //!< permit broadcasts of the Characteristic Value
#define CHAR_PROP_READ 0x02 //!< permit reads of the Characteristic Value
#define CHAR_PROP_WRITE_WITHOUT_RSP                                            \
  0x04 //!< Permit writes of the Characteristic Value without response
#define CHAR_PROP_WRITE                                                        \
  0x08 //!< Permit writes of the Characteristic Value with response
#define CHAR_PROP_NOTIFY                                                       \
  0x10 //!< Permit notifications of a Characteristic Value without
       //!< acknowledgement
#define CHAR_PROP_INDICATE                                                     \
  0x20 //!< Permit indications of a Characteristic Value with acknowledgement
#define CHAR_PROP_AUTHEN                                                       \
  0x40 //!< permit signed writes to the Characteristic Value
#define CHAR_PROP_EXTENDED                                                     \
  0x80 //!< additional characteristic properties are defined
/** @} end of group GATT_Characteristic_Property */

/** @addtogroup GATT_CCCC_Bits Client CharacteristicConfiguration bits
 * @{
 */
#define CLIENT_CHAR_CFG_NOTI                                                   \
  0x0001 //!< permit broadcasts of the Characteristic Value
...
/** @addtogroup GATT_Char_Cfg_Bit_length GATT characteristic configuration Bits
 * length
 * @{
 */
...
void att_req_find_by_type(u8 *dat, u16 start_attHandle, u16 end_attHandle,
                          u8 *uuid, u8 *attr_value, int len);
// 0x08: ATT_OP_READ_BY_TYPE_REQ
void att_req_read_by_type(u8 *dat, u16 start_attHandle, u16 end_attHandle,
...
void att_req_read_by_group_type(u8 *dat, u16 start_attHandle, u16 end_attHandle,
                                u8 *uuid, int uuid_len);
...
static inline void blt_att_setEffectiveMtuSize(u16 connHandle,
                                               u8 effective_mtu) {
