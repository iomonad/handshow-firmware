...
typedef struct {
  ...
  u8 conn_terminate_pending; // terminate_pending = master_terminate ||
                             // slave_terminate
  ...
  u32 ll_remoteFeature; // feature mask <0:31>,  not only one for BLE master,
                        // use connHandle to identify
...
#define IS_LL_CONNECTION_VALID(handle)                                         \
  (bltc.connHandle == (handle) &&                                              \
...
u16 bls_ll_getConnectionInterval(
    void); // if return 0, means not in connection state
u16 bls_ll_getConnectionLatency(
    void); // if return 0, means not in connection state
u16 bls_ll_getConnectionTimeout(
    void); // if return 0, means not in connection state
...
// ble module event
ble_sts_t bls_hci_mod_setEventMask_cmd(u32 evtMask); // eventMask: module
                                                     // special
...
ble_sts_t bls_hci_receiveHostACLData(u16 connHandle, u8 PB_Flag, u8 BC_Flag,
                                     u8 *pData);
...
static inline void blt_save_snnesn() {
  // SN									NESN
  blt_bb.sn_nesn =
      ((REG_ADDR8(0xf22) & BIT(0)) << 4) | ((REG_ADDR8(0xf23) & BIT(4)) << 1);
}
