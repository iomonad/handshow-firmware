...
#define BLE_EXT_ADV_PROP_CONNECTABLE_SCANNABLE                                 \
  (0x0003) // BLE_EXT_ADV_PROP_CONNECTABLE | BLE_EXT_ADV_PROP_SCANNABLE
#define BLE_EXT_ADV_PROP_LEGACY_SCANNABLE                                      \
  (0x0012) // BLE_EXT_ADV_PROP_LEGACY | BLE_EXT_ADV_PROP_SCANNABLE
#define BLE_EXT_ADV_PROP_LEGACY_DIRECTED                                       \
  (0x0014) // BLE_EXT_ADV_PROP_LEGACY | BLE_EXT_ADV_PROP_DIRECTED
#define BLE_EXT_ADV_PROP_LEGACY_HD_DIRECTED                                    \
  (0x0018) // BLE_EXT_ADV_PROP_LEGACY | BLE_EXT_ADV_PROP_HD_DIRECTED
#define BLE_EXT_ADV_PROP_LEGACY_CONNECTABLE_SCANNABLE                          \
  (0x0013) // BLE_EXT_ADV_PROP_LEGACY | BLE_EXT_ADV_PROP_CONNECTABLE |
           // BLE_EXT_ADV_PROP_SCANNABLE
...
/******************************************************  ADV_EXT_IND
************************************************************ AdvA  TargetA  ADI
Aux	Sync	Tx   ACAD	AdvData			Structure Ptr  	Info
Power Non-Connectable Non-Scannable   Undirected without AUX
M		 X		X	   X	 X		X	   X
X			rf_pkt_adv_ext_ind_1

Non-Connectable
Non-Scannable   Undirected with    AUX			 X 	 	 X
M	   M	 X		X	   X	 X
rf_pkt_adv_ext_ind_2

Non-Connectable
Non-Scannable	Directed   without AUX			 M		 M
X	   X	 X		X	   X	 X
rf_pkt_adv_ext_ind_1

Non-Connectable
Non-Scannable	Directed   with	   AUX			 X	 	 X
M	   M	 X		X	   X	 X
rf_pkt_adv_ext_ind_2


Connectable 	Undirected					 	 X
X		M	   M	 X		X	   X	 X
rf_pkt_adv_ext_ind_2

Connectable 	Directed					 	 X
X		M	   M	 X		X	   X	 X
rf_pkt_adv_ext_ind_2

Scannable 		Undirected				     	 X
X		M	   M	 X		X	   X	 X
rf_pkt_adv_ext_ind_2

Scannable 		Directed
X		 X		M	   M	 X		X	   X
X			rf_pkt_adv_ext_ind_2
*********************************************************************************************************************************/
typedef struct {
  u32 dma_len;
...
/******************************************************  AUX_ADV_IND
************************************************************ AdvA  TargetA  ADI
Aux	Sync	Tx   ACAD	AdvData Ptr  	Info   Power Non-Connectable
Non-Scannable   Undirected with    AUX			 M 	 	 X
M	   O	 X		X	   X	 O
rf_pkt_aux_adv_ind_1

Non-Connectable
Non-Scannable	Directed   with	   AUX			 M	 	 M
M	   O	 X		X	   X	 O
rf_pkt_aux_adv_ind_2

Connectable 	Undirected					 	 M
X		M	   X	 X		X	   X	 O
rf_pkt_aux_adv_ind_1

Connectable 	Directed					 	 M
M		M	   X	 X		X	   X	 O
rf_pkt_aux_adv_ind_2

Scannable 		Undirected				     	 M
X		M	   X	 X		X	   X	 X
rf_pkt_aux_adv_ind_1

Scannable 		Directed
M		 M		M	   X	 X		X	   X
X			rf_pkt_aux_adv_ind_2
*********************************************************************************************************************************/
typedef struct {
  u32 dma_len;
...
#define MAX_ADVDATA_NUM_AUX_SCANRSP                                            \
  247 // 255 - 8( Extended Header Length(1) + Extended Header Flag(1) + AdvA(6)
      // ) = 247
...
Chained data after AUX_ADV_IND  :				 X X
M	   O	 X		X	   X	 O
rf_pkt_aux_chain_ind_1

Chained data after AUX_SCAN_RSP :				 X X
X	   O	 X		X	   X	 O
rf_pkt_aux_chain_ind_2
*********************************************************************************************************************************/
typedef struct {
  u32 dma_len;
...
#define MAX_ADVDATA_NUM_AUX_CHAIN_IND_1                                        \
  251 // 255 - 4( Extended Header Length(1) + Extended Header Flag(1) + ADI(2) )
      // = 251
...
/********************* Macro & Enumeration variables for Stack, user can not
 * use!!!!  **********************/
...
#define TX_TX_DELAY_US                                                         \
  120 // TX settle(74 uS) + 5  preamble(40uS) + some other baseband cost,   and
      // be 30*N
