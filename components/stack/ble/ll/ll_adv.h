...
typedef enum {
  ...
  ADV_TYPE_CONNECTABLE_DIRECTED_HIGH_DUTY =
      0x01,                             // ADV_INDIRECT_IND (high duty cycle)
  ...
  ADV_TYPE_CONNECTABLE_DIRECTED_LOW_DUTY =
      0x04, // ADV_INDIRECT_IND (low duty cycle)
} adv_type_t;
...
typedef enum advChannelPDUType_e {
  ...
  LL_TYPE_AUX_CONNNECT_REQ = 0x05,
  LL_TYPE_ADV_SCAN_IND = 0x06,

  LL_TYPE_ADV_EXT_IND = 0x07,
  LL_TYPE_AUX_ADV_IND = 0x07,
  LL_TYPE_AUX_SCAN_RSP = 0x07,
  LL_TYPE_AUX_SYNC_IND = 0x07,
  LL_TYPE_AUX_CHAIN_IND = 0x07,
...
typedef int (*ll_adv2conn_callback_t)(
    u8 *, bool); // rcvd conn_req, adv state to conn state
...
ble_sts_t blc_ll_setAdvParamInConnSlaveRole(
    u8 *adv_data, u8 advData_len, u8 *scanRsp_data, u8 scanRspData_len,
    adv_type_t advType, own_addr_type_t ownAddrType, u8 adv_channelMap,
    adv_fp_type_t advFilterPolicy);
...
static inline u8 blt_ll_getOwnAddrType(void) { return blta.own_addr_type; }
