...
#if (RAMCODE_OPTIMIZE_CONN_POWER_NEGLECT_ENABLE ||                             \
     BLS_BLE_RF_IRQ_TIMING_EXTREMELY_SHORT_EN)
...
typedef enum {
  ...
  ADV_FP_NONE =
      ADV_FP_ALLOW_SCAN_ANY_ALLOW_CONN_ANY, // adv filter policy set to zero,
                                            // not use whitelist
...
typedef enum {
  ...
  SCAN_FP_ALLOW_UNDIRECT_ADV =
      0x02, // and direct adv address match initiator's resolvable private MAC
  SCAN_FP_ALLOW_ADV_WL_DIRECT_ADV_MACTH =
      0x03, // and direct adv address match initiator's resolvable private MAC
...
ble_sts_t ll_resolvingList_add(u8 peerIdAddrType, u8 *peerIdAddr, u8 *peer_irk,
                               u8 *local_irk);
...
ble_sts_t ll_resolvingList_getPeerResolvableAddr(
    u8 peerIdAddrType, u8 *peerIdAddr,
    u8 *peerResolvableAddr); // not available now
ble_sts_t ll_resolvingList_getLocalResolvableAddr(
    u8 peerIdAddrType, u8 *peerIdAddr,
    u8 *LocalResolvableAddr); // not available now

ble_sts_t ll_resolvingList_setAddrResolutionEnable(u8 resolutionEn);

ble_sts_t ll_resolvingList_setResolvablePrivateAddrTimer(
    u16 timeout_s); // not available now
