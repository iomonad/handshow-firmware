...
#define CHARACTERISTIC_UUID_HID_BOOT_KEY_INPUT                                 \
  0x2A22 //!< HID Boot Keyboard Input Report
#define CHARACTERISTIC_UUID_HID_BOOT_KEY_OUTPUT                                \
  0x2A32 //!< HID Boot Keyboard Output Report
#define CHARACTERISTIC_UUID_HID_BOOT_MOUSE_INPUT                               \
...
#define HID_REPORT_ID_CONSUME_CONTROL_INPUT                                    \
  2                                   //!< Consumer Control input report ID
...
/**
 *  @brief  Definition client characterist configuration of report changed
 * callback function type for user application
 */
typedef void (*hids_clientCharCfgChangeCb_t)(clientCharCfgChange_t *cccVal);

/**
 *  @brief  Definition report value changed callback function type for user
 * application
 */
 ...
 */
typedef struct {
  hids_reportChangeCb_t
      reportChangeCbFunc; //!< report value changed callback function
  hids_protoModeChangeCb_t
      protoModeChangeCbFunc; //!< protocol mode value changed callback function
  hids_ctrlPointChangeCb_t
      ctrlPointChangeCbFunc; //!< control point value changed callback function
  hids_clientCharCfgChangeCb_t
      cccChangedCbFunc; //!< ccc of report changed callback function
} hids_callbacks_t;
