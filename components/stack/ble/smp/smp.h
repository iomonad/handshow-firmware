...
typedef struct {
  ...
  u32 bond_flash_idx[SMP_BONDING_DEVICE_MAX_NUM]; // mark paired slave mac
                                                  // address in flash
...
typedef struct {
  ...
  u32 ecdh_debug_mode; // 1:debug_mode,0:ecdh public/private key pairs
                       // distribute
  ...
  u8 tk_status;
  u8 peerKey_mask; // determine which peer key to store   <0>: EncKey  <1>:IdKey
                   // <2>:Sign, NOTE: not used now in code
...
typedef struct {
  ...
  u8 peer_id_address_type; // peer identity address information in key
                           // distribution, used to identify
...
void blc_smp_enableAuthMITM(
    int MITM_en); // enable authentication MITM protection
void blc_smp_enableOobAuthentication(int OOB_en); // enable OOB authentication
void blc_smp_setIoCapability(
    io_capability_t ioCapablility); // set IO capability for passKey
...
void blc_smp_setNumericComparisonResult(
    bool YES_or_NO); // numeric compare confirm, 1: YES  0: NO
...
static inline int blc_smp_isParingBusy(void) { return blc_smpMng.paring_busy; }
...
smp_keyDistribution_t blc_smp_setInitiatorKey(u8 LTK_distributeEn,
                                              u8 IRK_distributeEn,
                                              u8 CSRK_DistributeEn);
smp_keyDistribution_t blc_smp_setResponderKey(u8 LTK_distributeEn,
                                              u8 IRK_distributeEn,
                                              u8 CSRK_DistributeEn);

int blc_stack_smp_setSecurityParamters(bonding_mode_t mode,
                                       paring_methods_t method, int MITM_en,
                                       int OOB_en,
                                       io_capability_t ioCapablility,
                                       int keyPress_en);
