...
typedef struct {
  ...
  u32 bond_flash_idx[PAIR_SLAVE_MAX_NUM]; // mark paired slave mac address in
                                          // flash
  mac_adr_t bond_device[PAIR_SLAVE_MAX_NUM];
} bond_slave_t;

#define SLAVE_TRIGGER_SMP_FIRST_PAIRING                                        \
  0 // first pair, slave send security_request to trigger master's
    // pairing&encryption
#define MASTER_TRIGGER_SMP_FIRST_PAIRING BIT(0)

#define SLAVE_TRIGGER_SMP_AUTO_CONNECT                                         \
  0 // auto connect, slave send security_request to trigger master's encryption
