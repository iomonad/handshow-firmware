 ...
 *(Shanghai) Co., Ltd. and the licensee or the terms described here-in. This
 *heading MUST NOT be removed from this file.
 *
 ...
 *	ADC resolution
 */
typedef enum { RES8, RES10, RES12, RES14 } ADC_ResTypeDef;
...
static inline void adc_enable_clk_24m_to_sar_adc(unsigned int en) {
  if (en) {
    analog_write(areg_clk_setting,
                 analog_read(areg_clk_setting) | FLD_CLK_24M_TO_SAR_EN);
  } else {
    analog_write(areg_clk_setting,
                 analog_read(areg_clk_setting) & ~FLD_CLK_24M_TO_SAR_EN);
  }
...
/**
 * @brief      This function sets adc sample clk. adc sample clk = 24M/(1+div)
 * div: 0~7.
 * @param[in]  div - the divider of adc sample clock.
 * @return     none
 */
static inline void adc_set_sample_clk(unsigned char div) {
  // afe_0xF4<7:3> is reserved, so no need to care its value (confirmed by
  // junwei & congqing 20190805)
...
afe_0xE7
    ...
    BIT<5:4>  adc_vrefm, Select VREF for Misc channel
                        ...
                        0x1: 0.9V
                        0x2: 1.2V
                        0x3: VBAT/N (N=2/3/4, when afe_0xF9<3:2> is set as
0x3/0x2/0x1).

        notice that: when choose ADC_VREF_VBAT_N, must use
adc_set_vref_vbat_divider to config N(1/2/3/4)
 ...
 * @return     none
 */
static inline void adc_set_vref(ADC_RefVolTypeDef vRef_L,
...
static inline void adc_set_vref_chn_left(ADC_RefVolTypeDef v_ref) {
  analog_write(
      areg_adc_vref,
      ((analog_read(areg_adc_vref) & (~FLD_ADC_VREF_CHN_L)) | (v_ref)));
}
...

{
  analog_write(
      areg_adc_vref,
      ((analog_read(areg_adc_vref) & (~FLD_ADC_VREF_CHN_R)) | (v_ref << 2)));
}
...

{
  analog_write(
      areg_adc_vref,
      ((analog_read(areg_adc_vref) & (~FLD_ADC_VREF_CHN_M)) | (v_ref << 4)));
}
 ...
 * @return none
 */
static inline void adc_set_ain_chn_misc(ADC_InputPchTypeDef p_ain,
 ...
 * @return none
 */
static inline void adc_set_ain_chn_left(ADC_InputPchTypeDef p_ain,
                                        ADC_InputNchTypeDef n_ain)
 ...
 * @return none
 */
static inline void adc_set_ain_chn_right(ADC_InputPchTypeDef p_ain,
                                         ADC_InputNchTypeDef n_ain)
...
/**
 * @brief      This function sets ADC analog negative input channel for the MISC
 * channel
...
{
  analog_write(areg_adc_ain_chn_misc,
               (analog_read(areg_adc_ain_chn_misc) & (~FLD_ADC_AIN_NEGATIVE)) |
                   (v_ain));
}
/**
 * @brief      This function sets ADC analog positive input channel for the MISC
 * channel
...
{
  analog_write(areg_adc_ain_chn_misc,
               (analog_read(areg_adc_ain_chn_misc) & (~FLD_ADC_AIN_POSITIVE)) |
                   (v_ain << 4));
}

/**
 * @brief      This function sets ADC analog negative input channel for the L
 * channel
 * @param[in]  v_ain - enum variable of ADC analog negative input.
 * @return     none
 */
...
{
  analog_write(areg_adc_ain_chn_left,
               (analog_read(areg_adc_ain_chn_left) & (~FLD_ADC_AIN_NEGATIVE)) |
                   (v_ain));
}

/**
 * @brief      This function sets ADC analog positive input channel for the L
 * channel
 * @param[in]  v_ain - enum variable of ADC analog positive input.
 * @return     none
 */
...
{
  analog_write(areg_adc_ain_chn_left,
               (analog_read(areg_adc_ain_chn_left) & (~FLD_ADC_AIN_POSITIVE)) |
                   (v_ain << 4));
}

/**
 * @brief      This function sets ADC analog negative input channel for the R
 * channel
 * @param[in]  v_ain - enum variable of ADC analog negative input.
 * @return     none
 */
...
{
  analog_write(areg_adc_ain_chn_right,
               (analog_read(areg_adc_ain_chn_right) & (~FLD_ADC_AIN_NEGATIVE)) |
                   (v_ain));
}

/**
 * @brief      This function sets ADC analog positive input channel for the R
 * channel
 * @param[in]  v_ain - enum variable of ADC analog positive input.
 * @return     none
 */
...
{
  analog_write(areg_adc_ain_chn_right,
               (analog_read(areg_adc_ain_chn_right) & (~FLD_ADC_AIN_POSITIVE)) |
                   (v_ain << 4));
}
...
afe_0xEC
    BIT<1:0>  adc_resm, Set resolution for Misc channel
                         ...
                         If RNS mode is set, resolution is set to 1. Otherwise
ADC mode setting is: 0x0: 8bits 0x1: 10bits 0x2: 12bits 0x3: 14bits

        BIT<3:2>  Reserved
        BIT<4>    adc_en_diffl,  Select single-end or differential input mode
for left channel. BIT<5>    adc_en_diffr,  Select single-end or differential
input mode for right channel. BIT<6>    adc_en_diffm,  Select single-end or
differential input mode for Misc channel. BIT<7>    Reserved
 *************************************************************************************/
...
static inline void adc_set_resolution_chn_left(ADC_ResTypeDef v_res) {
  analog_write(areg_adc_res_l_r,
               (analog_read(areg_adc_res_l_r) & (~FLD_ADC_RES_L)) | (v_res));
}
...
{
  analog_write(areg_adc_res_l_r,
               (analog_read(areg_adc_res_l_r) & (~FLD_ADC_RES_R)) |
                   (v_res << 4));
}
...
static inline void adc_set_resolution_chn_misc(ADC_ResTypeDef v_res) {
  analog_write(anareg_adc_res_m,
               (analog_read(anareg_adc_res_m) & (~FLD_ADC_RES_M)) | (v_res));
}
...
static inline void adc_set_input_mode_chn_left(ADC_InputModeTypeDef m_input) {
  if (m_input) { // differential mode
    analog_write(anareg_adc_res_m,
                 (analog_read(anareg_adc_res_m) | FLD_ADC_EN_DIFF_CHN_L));
...

{
  if (m_input) { // differential mode
    analog_write(anareg_adc_res_m,
                 analog_read(anareg_adc_res_m) | FLD_ADC_EN_DIFF_CHN_R);
  } else { // single-ended mode
    analog_write(anareg_adc_res_m,
                 analog_read(anareg_adc_res_m) & (~FLD_ADC_EN_DIFF_CHN_R));
  }
...
static inline void adc_set_input_mode_chn_misc(ADC_InputModeTypeDef m_input) {
  if (m_input) { // differential mode
    analog_write(anareg_adc_res_m,
                 analog_read(anareg_adc_res_m) | FLD_ADC_EN_DIFF_CHN_M);
  } else { // single-ended mode
    analog_write(anareg_adc_res_m,
                 analog_read(anareg_adc_res_m) & (~FLD_ADC_EN_DIFF_CHN_M));
  }
...
/**
 * @brief      This function sets ADC sample time(the number of adc clocks for
 * sample cycles) for the MISC channel.
...
static inline void adc_set_tsample_cycle_chn_misc(ADC_SampCycTypeDef adcST) {
  // ana_ee<7:4> is reserved, so no need care its value (confirmed by junwei &
  // congqing 20190805) analog_write(areg_adc_tsmaple_m,
  // (analog_read(areg_adc_tsmaple_m)&(~FLD_ADC_TSAMPLE_CYCLE_CHN_M)) | (adcST)
  // );
...
/**************************************************************************************
afe_0xED
    BIT<3:0>  adc_tsampl, Number of ADC clock cycles in sampling phase for left
channel to stabilize the input before sampling BIT<7:4>  adc_tsampr, Number of
ADC clock cycles in sampling phase for right channel to stabilize the input
before sampling

afe_0xEE
    BIT<3:0>  adc_tsampm, Number of ADC clock cycles in sampling phase for Misc
channel to stabilize the input before sampling
...
/**
 * @brief      This function sets ADC sample time(the number of adc clocks for
 * sample cycles) for the L channel.
 * @param[in]  adcST - enum variable of adc sample cycles.
 * @return     none
 */
...
{
  analog_write(areg_adc_tsmaple_l_r, (analog_read(areg_adc_tsmaple_l_r) &
...
/**
 * @brief      This function sets ADC sample time(the number of adc clocks for
 * sample cycles) for the R channel.
 * @param[in]  adcST - enum variable of adc sample cycles.
 * @return     none
 */
...
afe_0xF1
        ...
        r_max_mc[9:0]: serves to set length of ¡°capture¡± state for RNS and
Misc channel. r_max_c[9:0]:  serves to set length of ¡°capture¡± state for left
and right channel. r_max_s:       serves to set length of ¡°set¡± state for
left, right and Misc channel.

        Note: State length indicates number of 24M clock cycles occupied by the
state.
 *************************************************************************************/
...
static inline void adc_set_length_set_state(unsigned char r_max_s) {
  analog_write(areg_r_max_s,
               (analog_read(areg_r_max_s) & (~FLD_R_MAX_S)) | (r_max_s));
}

/**
 * @brief      This function sets length of each "capture" state for MISC
 * channel.
 * @param[in]  r_max_mc[9:0] - variable of length of "capture" state for MISC
 * channel.
 * @return     none
 ...
 */
static inline void
adc_set_length_capture_state_for_chn_misc_rns(unsigned short r_max_mc) {
  analog_write(areg_r_max_mc, (r_max_mc & 0x0ff));
  analog_write(areg_r_max_s, ((analog_read(areg_r_max_s) & (~FLD_R_MAX_MC1)) |
                              ((r_max_mc & 0x3ff) >> 8) << 6));
}

/**
 * @brief      This function sets length of each "capture" state for R and L
 * channel.
 * @param[in]  r_max_c[9:0] - variable of length of "capture" state for R and L
 * channel.
 * @return     none
 ...
 */
static inline void
adc_set_length_capture_state_for_chn_left_right(unsigned short r_max_c) {
  analog_write(areg_r_max_c, r_max_c & 0xff);
  analog_write(areg_r_max_s, (analog_read(areg_r_max_s) & (~FLD_R_MAX_C1)) |
                                 (((r_max_c & 0x3ff) >> 8) << 4));
}

/**
 * @brief This function serves to set state and capture_state length.
 * @param[in]   r_max_mc - Value of length of "capture" state for RNS & MISC
 * channel.
 * @param[in]   r_max_c - Value of length of "capture" state for L & R channel.
 * @param[in]   r_max_s - Value of length of "set" state for L & R & MISC
 * channel.
 * @return none
 */
static inline void adc_set_state_length(unsigned short R_max_mc,
                                        unsigned short R_max_c,
                                        unsigned char R_max_s) {
  analog_write(areg_r_max_mc, R_max_mc);
  analog_write(areg_r_max_c, R_max_c);
  analog_write(areg_r_max_s, ((R_max_mc >> 8) << 6) | ((R_max_c >> 8) << 4) |
                                 (R_max_s & FLD_R_MAX_S));
}
...
afe_0xF2
    ...
    BIT<1>   r_en_right   Enable right channel. 1: enable;  0: disable
        ...
        BIT<6:4> r_max_scnt   Set total length for sampling state machine (i.e.
max state index)
 *************************************************************************************/
 ...
 * @return     none
 */
static inline void adc_set_chn_enable_and_max_state_cnt(ADC_ChTypeDef ad_ch,
                                                        unsigned char s_cnt) {
...
static inline void adc_set_max_state_cnt(unsigned char s_cnt) {
  analog_write(areg_adc_chn_en,
               (analog_read(areg_adc_chn_en) & (~FLD_ADC_MAX_SCNT)) |
                   ((s_cnt & 0x07) << 4));
}

/**************************************************************************************
afe_0xFA
                BIT<1:0>  adc_itrim_preamp, Comparator preamp bias current
trimming BIT<3:2>  adc_itrim_vrefbuf, Vref buffer bias current trimming BIT<5:4>
adc_itrim_vcmbuf, Vref buffer bias current trimming
...
/**
 * @brief      This function sets pre-scaling for comparator preamp bias current
 * trimming.
 * @param[in]  bias - enum variable of current trimming.
 * @return     none
 */
static inline void adc_set_itrim_preamp(ADC_Cur_TrimTypeDef bias) {
  analog_write(areg_ain_scale,
               (analog_read(areg_ain_scale) & (~FLD_ADC_ITRIM_PREAMP)) |
                   (bias << 0));
}

/**
 * @brief      This function sets pre-scaling for Vref buffer bias current
 * trimming.
 * @param[in]  bias - enum variable of current trimming.
 * @return     none
 */
static inline void adc_set_itrim_vrefbuf(ADC_Cur_TrimTypeDef bias) {
  analog_write(areg_ain_scale,
               (analog_read(areg_ain_scale) & (~FLD_ADC_ITRIM_VREFBUF)) |
                   (bias << 2));
}

/**
 * @brief      This function sets pre-scaling for Vref(Vcmbuf) buffer bias
 * current trimming.
 * @param[in]  bias - enum variable of current trimming.
 * @return     none
 */
static inline void adc_set_itrim_vcmbuf(ADC_Cur_TrimTypeDef bias) {
  analog_write(areg_ain_scale,
               (analog_read(areg_ain_scale) & (~FLD_ADC_ITRIM_VCMBUF)) |
                   (bias << 4));
}
...
static inline void adc_set_left_boost_bias(ADC_Gain_BiasTypeDef bias) {
  analog_write(areg_adc_pga_boost,
               (analog_read(areg_adc_pga_boost) & (~FLD_PGA_ITRIM_BOOST_L)) |
                   (bias << 4) | FLD_PGA_CAP_TRIM_EN_L);
}
...
static inline void adc_set_right_boost_bias(ADC_Gain_BiasTypeDef bias) {
  analog_write(areg_adc_pga_boost,
               (analog_read(areg_adc_pga_boost) & (~FLD_PGA_ITRIM_BOOST_R)) |
                   (bias << 6) | FLD_PGA_CAP_TRIM_EN_R);
}
 ...
 * @return     none
 */
static inline void adc_set_left_right_gain_bias(ADC_Gain_BiasTypeDef bias_L,
                                                ADC_Gain_BiasTypeDef bias_R) {
  analog_write(areg_adc_pga_ctrl, (analog_read(areg_adc_pga_ctrl) & 0xf0) |
                                      (bias_L | bias_R << 2));
}
...
static inline void adc_set_left_gain_bias(ADC_Gain_BiasTypeDef bias) {
  analog_write(areg_adc_pga_ctrl,
               (analog_read(areg_adc_pga_ctrl) & (~FLD_PGA_ITRIM_GAIN_L)) |
                   (bias));
}
...
static inline void adc_set_right_gain_bias(ADC_Gain_BiasTypeDef bias) {
  analog_write(areg_adc_pga_ctrl,
               (analog_read(areg_adc_pga_ctrl) & (~FLD_PGA_ITRIM_GAIN_R)) |
                   (bias << 2));
}
...
static inline void adc_set_mode(ADC_ModeTypeDef adc_m) {
  analog_write(areg_adc_pga_ctrl,
               (analog_read(areg_adc_pga_ctrl) & (~FLD_ADC_MODE)) | adc_m);
}
...
static inline void adc_power_on_sar_adc(unsigned char on_off) {
  analog_write(areg_adc_pga_ctrl,
               (analog_read(areg_adc_pga_ctrl) & (~FLD_SAR_ADC_POWER_DOWN)) |
                   (!on_off) << 5);
}
...
static inline void pga_left_chn_power_on(unsigned char on_off) {
  analog_write(areg_adc_pga_ctrl,
               (analog_read(areg_adc_pga_ctrl) & (~FLD_POWER_DOWN_PGA_CHN_L)) |
                   (!on_off) << 6);
}
...
static inline void pga_right_chn_power_on(unsigned char on_off) {
  analog_write(areg_adc_pga_ctrl,
               (analog_read(areg_adc_pga_ctrl) & (~FLD_POWER_DOWN_PGA_CHN_R)) |
                   (!on_off) << 7);
}

#define areg_fe                                                                \
  0xfe // 0xfe default value is 0xe5,for output audio, mast claer 0xfe<7:5>

/**
 * @brief      This function sets  ADC RNS channel source and random updata
 * type.
...
afe_0xF8<7:0>   adc_dat[15:8]  	Read only
                                                                [7]: vld, ADC
data valid status bit (This bit will be set as 1 at the end of capture state to
indicate the ADC data is valid, and will be cleared when set state starts.)
                                                                [6:0]:  Misc
adc_dat[14:8]
...
/**
 * @brief This function is used for IO port configuration of ADC IO port voltage
 * sampling.
...
/**
 * @brief This function is used for IO port configuration of ADC supply voltage
 * sampling.
 * @param[in]  pin - GPIO_PinTypeDef
 * @return none
 */
...
/**
 * @brief This function is used for ADC configuration of ADC IO voltage
 * sampling.
 * @param[in]   pin - GPIO_PinTypeDef
 * @return none
 */
...
/**
 * @brief This function is used for ADC configuration of ADC supply voltage
 * sampling.
 * @param[in]    pin - GPIO_PinTypeDef
 * @return none
 */
