...
enum {
  ...
  FLD_I2C_SLAVE_MAPPING = BIT(2), // write i2c data to predefined memory address
                                  // which set by other register
...
enum {
  ...
  FLD_SLAVE_SPI_IRQ =
      FLD_HOST_CMD_IRQ,       // both host write & read trigger this status
...
#define reg_gpio_irq_wakeup_en(i)                                              \
  REG_ADDR8(0x587 + ((i >> 8) << 3)) // reg_irq_mask: FLD_IRQ_GPIO_EN

#define reg_gpio_irq_risc0_en(i)                                               \
  REG_ADDR8(0x5b8 + (i >> 8)) // reg_irq_mask: FLD_IRQ_GPIO_RISC0_EN
#define reg_gpio_irq_risc1_en(i)                                               \
  REG_ADDR8(0x5c0 + (i >> 8)) // reg_irq_mask: FLD_IRQ_GPIO_RISC1_EN
...
enum {
  ...
  FLD_IRQ_HOST_CMD_EN = BIT(7), //< MIX = I2C/QDEC/SPI
...
#define reg_pwm_cycle(i)                                                       \
  REG_ADDR32(0x794 + (i << 2)) //<15:0>: TCMP 0~5  <31:16>: TMAX 0~5
...
#define reg_pwm_tcmp0_shadow                                                   \
  REG_ADDR16(0x7c4) //<15:0>: TCMP 0~5  <31:16>: TMAX 0~5
...
#define reg_dfifo0_l_level                                                     \
  REG_ADDR8(0xb0c) // dfifo0  low int threshold(wptr - rptr)
#define reg_dfifo0_h_level                                                     \
  REG_ADDR8(0xb0d) // dfifo0 high int threshold(wptr - rptr)
#define reg_dfifo1_h_level                                                     \
  REG_ADDR8(0xb0e) // dfifo1 high int threshold(wptr - rptr)
#define reg_dfifo2_h_level                                                     \
  REG_ADDR8(0xb0f) // dfifo2 high int threshold(wptr - rptr)
...
enum { FLD_AUD_LNR_VALID_SEL = BIT(0), FLD_AUD_CIC_MODE = BIT(3) };
...
#define reg_aud_alc_vol_l_chn                                                  \
  REG_ADDR8(0xb41) // default 0x00, will be 0x20 after ana_34 set to 0x80
#define reg_aud_alc_vol_r_chn                                                  \
  REG_ADDR8(0xb42) // default 0x00, will be 0x20 after ana_34 set to 0x80
enum {
  FLD_AUD_ALC_MIN_VOLUME_IN_DIGITAL_MODE = BIT_RNG(0, 5),
  FLD_AUD_ALC_MIN_PGA_IN_ANALOG_MODE = BIT_RNG(0, 6),
  // alc digital mode left/right channel regulate mode select: 1 for auto
  // regulate; 0 for manual regulate
...
#define reg_aud_alc_cfg                                                        \
  REG_ADDR8(0xb54) // default 0x00, will be 0x02 after ana_34 set to 0x80
enum {
  FLD_AUD_ALC_ANALOG_MODE_EN =
      BIT(0), // alc mode select:   1 for analog mode;  0 for digital mode
...
#define reg_pga_gain_l                                                         \
  REG_ADDR8(0xb5e) // used to check current left  channel gain in analog mode
                   // auto regulate
#define reg_pga_gain_r                                                         \
  REG_ADDR8(0xb5f) // used to check current right channel gain in analog mode
                   // auto regulate
