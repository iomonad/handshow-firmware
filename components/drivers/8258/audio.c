...
unsigned char AMIC_ADC_SampleLength[3] = {0xf0 /*96K*/, 0xac /*132K*/,
                                          0x75 /*192K*/};

unsigned char DMIC_CLK_Mode[RATE_SIZE] = {
    47 /*8k	1021.277*/, 47 /*16k	1021.277*/, 47 /*32k	1021.277*/};

unsigned char DMIC_CIC_Rate[RATE_SIZE] = {
    0x23 /*8k 7.978723*/, 0x22 /*16k 15.95745*/, 0x11 /*32k 31.91489*/};

unsigned char AMIC_CIC_Rate[RATE_SIZE] = {
    0xab /*8k  96/12	*/, 0x32 /*16k 96/6	*/, 0x42 /*32k 96/3	*/};

/*  matrix used under condition: I2S = 2 MHz  */
unsigned long ASDM_Rate_Matching[RATE_SIZE] = {
    0x00832001 /*8k */, 0x01063001 /*16k*/, 0x020C5001 /*32k*/};

/*  matrix used under condition: I2S = 2 MHz  */
unsigned long DSDM_Rate_Matching[RATE_SIZE] = {
    0x00820001 /*8 k*/, 0x01058001 /*16k*/, 0x020AF001 /*32k*/};
...
/**
 * 	@brief     audio amic initial function. configure ADC corresponding
 * parameters. set hpf,lpf and decimation ratio.
...
void audio_amic_init(AudioRate_Typedef Audio_Rate) {
  ...
  // adc state machine state cnt 2( "set" stage and "capture" state for left
  // channel)
  adc_set_chn_enable_and_max_state_cnt(
      ADC_LEFT_CHN, 2); // left channel enable and  state cnt= 2
  ...
  adc_set_input_mode(ADC_LEFT_CHN,
                     DIFFERENTIAL_MODE); // left channel differential mode
  adc_set_ain_channel_differential_mode(
      ADC_LEFT_CHN, PGA0P, PGA0N); // left channel positive and negative data in

  adc_set_ref_voltage(ADC_RIGHT_CHN, ADC_VREF_0P6V);

  adc_set_resolution(ADC_LEFT_CHN, RES14); // left channel resolution
  adc_set_tsample_cycle(ADC_LEFT_CHN, SAMPLING_CYCLES_6); // left channel
                                                          // tsample
  ...
  analog_write(areg_adc_pga_ctrl,
               MASK_VAL(FLD_PGA_ITRIM_GAIN_L, GAIN_STAGE_BIAS_PER150,
                        FLD_PGA_ITRIM_GAIN_R, GAIN_STAGE_BIAS_PER150,
                        FLD_ADC_MODE, 0, FLD_SAR_ADC_POWER_DOWN, 1,
                        FLD_POWER_DOWN_PGA_CHN_L, 0, FLD_POWER_DOWN_PGA_CHN_R,
                        1));

  WriteAnalogReg(0xfe, 0x05); // 0x80+126  = 0x05,0xfe default value is 0xe5,for
                              // output audio, must clear 0xfe<7:5>
  /***********************************PGA gain
   * setting*******************************************/
  ...
  reg_aud_alc_hpf_lpf_ctrl = MASK_VAL(
      FLD_AUD_IN_HPF_SFT, 0x0b, // different pcb may set different value.
      FLD_AUD_IN_HPF_BYPASS, 0, FLD_AUD_IN_ALC_BYPASS, 0, FLD_AUD_IN_LPF_BYPASS,
      0, FLD_DOUBLE_DOWN_SAMPLING_ON, (Audio_Rate == AUDIO_32K) ? 0 : 1);
  ...
  reg_aud_alc_vol_l_chn =
      MASK_VAL(FLD_AUD_ALC_MIN_VOLUME_IN_DIGITAL_MODE, 0x24,
               FLD_AUD_ALC_DIGITAL_MODE_AUTO_REGULATE_EN, 0);

  // 2. Dfifo setting
  reg_clk_en2 |= FLD_CLK2_DFIFO_EN; // enable dfifo clock, this will be
                                    // initialed in cpu_wakeup_int()
  ...
  reg_dfifo_ain = MASK_VAL(
      FLD_AUD_DMIC0_DATA_IN_RISING_EDGE, AUDIO_DMIC_DATA_IN_FALLING_EDGE,
      FLD_AUD_INPUT_SELECT, AUDIO_INPUT_AMIC, FLD_AUD_INPUT_MONO_MODE, 1,
...
/**
 * @brief     audio DMIC init function, config the speed of DMIC and downsample
 * audio data to required speed. actually audio data is dmic_speed/d_samp.
...
void audio_dmic_init(AudioRate_Typedef Audio_Rate) {
  ...
  reg_dfifo_ain = MASK_VAL(
      FLD_AUD_DMIC0_DATA_IN_RISING_EDGE, AUDIO_DMIC_DATA_IN_RISING_EDGE,
      FLD_AUD_INPUT_SELECT, AUDIO_INPUT_DMIC, FLD_AUD_INPUT_MONO_MODE, 1,
      FLD_AUD_DECIMATION_FILTER_BYPASS, 0, FLD_AUD_DMIC_RISING_EDGE_BYPASS, 0,
  ...
  reg_aud_alc_hpf_lpf_ctrl = MASK_VAL(
      FLD_AUD_IN_HPF_SFT, 0x0b, // different pcb may set different value.
      FLD_AUD_IN_HPF_BYPASS, 0, FLD_AUD_IN_ALC_BYPASS, 1, FLD_AUD_IN_LPF_BYPASS,
      1, FLD_DOUBLE_DOWN_SAMPLING_ON, 1);
...
/**
 * @brief     audio USB init function, config the speed of DMIC and downsample
 * audio data to required speed. actually audio data is dmic_speed/d_samp.
 * @param[in] Audio_Rate  - audio rate.
 * @return    none.
 */
void audio_usb_init(AudioRate_Typedef Audio_Rate) {
  ...
  reg_dfifo_ain = MASK_VAL(
      FLD_AUD_DMIC0_DATA_IN_RISING_EDGE, AUDIO_DMIC_DATA_IN_RISING_EDGE,
      FLD_AUD_INPUT_SELECT, AUDIO_INPUT_USB, FLD_AUD_INPUT_MONO_MODE, 0,
      FLD_AUD_DECIMATION_FILTER_BYPASS, 1, FLD_AUD_DMIC_RISING_EDGE_BYPASS, 0,
      FLD_AUD_DMIC_FALLING_EDGE_BYPASS, 0);
  ...
  reg_aud_alc_hpf_lpf_ctrl = MASK_VAL(
      FLD_AUD_IN_HPF_SFT, 0x0b, // different pcb may set different value.
      FLD_AUD_IN_HPF_BYPASS, 1, FLD_AUD_IN_ALC_BYPASS, 1, FLD_AUD_IN_LPF_BYPASS,
      1, FLD_DOUBLE_DOWN_SAMPLING_ON, 0);
  /*******1.Dmic setting for audio input**************************/
  reg_audio_ctrl = AUDIO_OUTPUT_OFF;
...
/**
 * @brief     audio buff init function, config the speed of DMIC and downsample
 * audio data to required speed. actually audio data is dmic_speed/d_samp.
 * @param[in] Audio_Rate  - audio rate.
 * @return    none.
 */
...
/**
 *	@brief	   sdm setting function, enable or disable the sdm output,
 *configure SDM output paramaters.
 *	@param[in]	InType -	  SDM input type, such as
 *AMIC,DMIC,I2S_IN,USB_IN.
 *	@param[in]	Audio_Rate - audio sampling rate, such as 16K,32k etc.
 *	@param[in]	audio_out_en - audio output enable or disable set, '1'
 *enable audio output; '0' disable output.
 *	@return	none
 */
void audio_set_sdm_output(AudioInput_Typedef InType,
  ...
  if (audio_out_en) {
    ...
    reg_pwm_ctrl = MASK_VAL(FLD_PWM_MULTIPLY2, 0, FLD_PWM_ENABLE, 0,
                            FLD_LINER_INTERPOLATE_EN, 1, FLD_LEFT_SHAPING_EN, 0,
    ...
    reg_pn1_left =
        MASK_VAL(PN1_LEFT_CHN_BITS, 6, PN2_LEFT_CHN_EN, 0, PN1_LEFT_CHN_EN, 0);
    reg_pn2_left = MASK_VAL(PN2_LEFT_CHN_BITS, 6, PN2_RIGHT_CHN_EN, 0,
                            PN1_RIGHT_CHN_EN, 0);
    reg_pn1_right = MASK_VAL(PN1_RIGHT_CHN_BITS, 6, CLK2A_AUDIO_CLK_EN, 0,
                             EXCHANGE_SDM_DATA_EN, 0);
    reg_pn2_right = MASK_VAL(PN2_RIGHT_CHN_BITS, 6, SDM_LEFT_CHN_CONST_EN, 0,
    ...
    if (InType == I2S_IN) {
      reg_audio_ctrl = (FLD_AUDIO_MONO_MODE | FLD_AUDIO_SDM_PLAYER_EN |
                        FLD_AUDIO_I2S_RECORDER_EN | FLD_AUDIO_I2S_INTERFACE_EN);
...
/*
 *	The software control interface may be operated using either a 3-wire
 *(SPI-compatible) or 2-wire MPU interface. Selection of interface format is
 *achieved by setting the state of the MODE pin. MODE	0	2 wire
 *default(hardware) 1	3 wire In 2-wire mode, the state of CSB pin allows the
 *user to select one of two addresses. CSB		0	0011010
 *default(hardware) 1   0011011
 *
 *	note:
 *		The WM8731 is not a standard I2C interface, so the Kite standard
 *I2C driver is not used.
 */
...
unsigned char I2S_To_HPout_CMD_TAB[7][2] = {
    {WM8731_RESET_CTRL, 0x00},
...
void audio_set_codec(I2C_GPIO_GroupTypeDef i2c_pin_group,
                     CodecMode_Typedef CodecMode, unsigned sysclk) {
  ...
  i2c_master_init(
      0x34,
      (unsigned char)(sysclk / (4 * 200000))); // i2c clock 200K, only master
                                               // need set i2c clock
...
/**
 * @brief     audio I2S init in function, config the speed of i2s and MCLK to
 * required speed.
 * @param[in] none.
 * @return    none.
 */
void audio_i2s_init(void) {
  ...
  reg_dfifo_ain = MASK_VAL(
      FLD_AUD_DMIC0_DATA_IN_RISING_EDGE, AUDIO_DMIC_DATA_IN_RISING_EDGE,
      FLD_AUD_INPUT_SELECT, AUDIO_INPUT_I2S, FLD_AUD_INPUT_MONO_MODE, 1,
      FLD_AUD_DECIMATION_FILTER_BYPASS, 1, FLD_AUD_DMIC_RISING_EDGE_BYPASS, 0,
      FLD_AUD_DMIC_FALLING_EDGE_BYPASS, 0);
...
/**
 *
 * @brief	   	i2s setting function, enable or disable the i2s output,
 * configure i2s output paramaters
 * @param[in] 	InType		- select audio input type including amic ,dmic
 * ,i2s and usb
 * @param[in] 	Audio_Rate 	- select audio rate, which will decide on which
 * adc sampling rate and relative decimation configuration will be chosen.
 * @return	  	none
 */
void audio_set_i2s_output(AudioInput_Typedef InType,
  ...
  if (InType == I2S_IN) {
    ...
    reg_audio_ctrl = (FLD_AUDIO_MONO_MODE | FLD_AUDIO_SDM_PLAYER_EN |
                      FLD_AUDIO_ISO_PLAYER_EN);
  } else {
    reg_audio_ctrl = (FLD_AUDIO_MONO_MODE | FLD_AUDIO_I2S_INTERFACE_EN |
                      FLD_AUDIO_I2S_PLAYER_EN);
  }
 ...
 * @return    none.
 */
void audio_set_usb_output(void) { write_reg8(0xb10, 0xf9); }
