...
flash_write_page(unsigned long addr, unsigned long len, unsigned char *buf);

/**
 * @brief This function reads the content from a page to the buf.
 * @param[in]   addr the start address of the page
 * @param[in]   len the length(in byte) of content needs to read out from the
 * page
 * @param[out]  buf the start address of the buffer
 * @return none
 */
_attribute_ram_code_ void flash_read_page(unsigned long addr, unsigned long len,
                                          unsigned char *buf);
...
/**
 * @brief  	Deep Power Down mode to put the device in the lowest consumption
 * mode it can be used as an extra software protection mechanism,while the
 * device is not in active use,since in the mode,  all write,Program and Erase
 * commands are ignored,except the Release from Deep Power-Down and Read Device
 * ID(RDI) command.This release the device from this mode
 * @param[in] none
 * @return none.
 */
...
/**
 * @brief		The Release from Power-Down or High Performance
 * Mode/Device ID command is a Multi-purpose command.it can be used to release
 * the device from the power-Down State or High Performance Mode or obtain the
 * devices electronic identification (ID)number.Release from Power-Down will
 * take the time duration of tRES1 before the device will resume normal
 * operation and other command are accepted.The CS# pin must remain high during
 * the tRES1(8us) time duration.
 * @param[in]   none
 * @return      none.
 */
...
/**
 * @brief 		 This function serves to read flash mid and uid,and
 check the correctness of mid and uid.
