...
#define PM_LONG_SLEEP_WAKEUP_EN                                                \
  0 // if user need to make MCU sleep for a long time that is more than 268s,
    // this macro need to be enabled and use "pm_long_sleep_wakeup" function

/**
 * @brief analog register below can store infomation when MCU in deepsleep mode
 * 	      store your information in these ana_regs before deepsleep by
 * calling analog_write function when MCU wakeup from deepsleep, read the
 * information by by calling analog_read function Reset these analog registers
 * only by power cycle
 */
...
/**
 * @brief these analog register can store data in deepsleep mode or deepsleep
 * with SRAM retention mode. Reset these analog registers by watchdog, chip
 * reset, RESET Pin, power cycle
 */
...
/**
 * @brief      This function serves to determine whether mcu is waked up from
 * deep retention.
 * @param[in]  none.
 * @return     1- yes , 0- no.
 */
 ...
 * @return     1- yes , 0- no.
 */
static inline int pm_is_deepPadWakeup(void) { return pmParam.is_pad_wakeup; }
 ...
 */
static inline int pm_get_wakeup_src(void) { return pmParam.wakeup_src; }
...
/**
 * @brief   This function serves to wake up cpu from stall mode by timer1 or RF
 * TX done irq.
 ...
 * @return  none.
 */
unsigned int cpu_stall(int WakeupSrc, unsigned int IntervalUs,
                       unsigned int sysclktick);

/**
 * @brief      This function configures a GPIO pin as the wakeup pin.
 * @param[in]  pin - the pin needs to be configured as wakeup pin
 * @param[in]  pol - the wakeup polarity of the pad pin(0: low-level wakeup, 1:
 * high-level wakeup)
 * @param[in]  en  - enable or disable the wakeup function for the pan pin(1:
 * Enable, 0: Disable)
 * @return     none
 */
...
/**
 * @brief   This function serves to recover system timer from tick of internal
 * 32k RC.
 * @param   none.
 * @return  none.
 */
...
/**
 * @brief   This function serves to recover system timer from tick of external
 * 32k crystal.
 * @param   none.
 * @return  none.
 */
...
/**
 * @brief      This function serves to set the working mode of MCU based on 32k
 * crystal,e.g. suspend mode, deepsleep mode, deepsleep with SRAM retention mode
 * and shutdown mode.
 * @param[in]  sleep_mode - sleep mode type select.
 * @param[in]  wakeup_src - wake up source select.
 * @param[in]  wakeup_tick - the time of short sleep, which means MCU can sleep
...
typedef int (*cpu_pm_handler_t)(SleepMode_TypeDef sleep_mode,
                                SleepWakeupSrc_TypeDef wakeup_src,
                                unsigned int wakeup_tick);
...
/**
 * @brief      This function serves to determine whether wake up source is
 * internal 32k RC.
 * @param[in]  none.
 * @return     none.
 */
static inline void blc_pm_select_internal_32k_crystal(void) {
  ...
  blt_miscParam.pm_enter_en = 1; // allow enter pm, 32k rc does not need to wait
                                 // for 32k clk to be stable
}
 ...
 * @return     indicate whether the cpu is wake up successful.
 */
int pm_long_sleep_wakeup(SleepMode_TypeDef sleep_mode,
                         SleepWakeupSrc_TypeDef wakeup_src,
