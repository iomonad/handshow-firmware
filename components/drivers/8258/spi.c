...
/**
 * @brief     This function selects a pin port for the SPI interface (master
 *mode)
 ...
 *input ,set 0 not as i5c input 5b6[7:4]  set spi or i2c output,5b6[5:4] PA spi
 *sel, 5b6[6] PB spi sel,5b6[7] PD spi_sel spi i2c mux sel: step1:close gpio
 *func; step2:set 0x5a8-0x5af sel spii2c func; step3:set 5b6[7:4] to sel spi or
 *i2c output; step4:set 5b7[7:0] to sel spi or i2c input; step5 if use pd[6]as
 *spi slave csn,need to open pd[2] gpio func,because pd[2]have high priority
 *than pd[6]
 */
...
/**
 * @brief     This function configures the clock and working mode for SPI
 * interface
...
void spi_master_init(unsigned char DivClock, SPI_ModeTypeDef Mode) {
  ...
  reg_spi_sp |= DivClock; // 0x0a: bit0~bit6 set spi clock ; spi clock=system
                          // clock/((DivClock+1)*2)
...
void spi_write(unsigned char *Cmd, int CmdLen, unsigned char *Data, int DataLen,
  ...
  reg_spi_ctrl &=
      ~FLD_SPI_RD; // enable write,0x09-bit3 : 0 for read ,1 for write
...
void spi_read(unsigned char *Cmd, int CmdLen, unsigned char *Data, int DataLen,
  ...
  /***when the read_bit was set 1,you can read 0x800008 to take eight clock
   * cycle***/
...
/**
 * @brief     This function selects a pin port for the SPI interface (slave
 *mode)
 * @param[in] PinGrp - the selected pin port
 * @return    none
 *  A3:5b7[0] set 1 as spi input,set 0 not as spi input ;5b7[4] set 1 as i2c
