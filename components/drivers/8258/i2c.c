 ...
 * @return     none
 * 	A3:5b7[0] set 1 as spi input,set 0 not as spi input ;5b7[4] set 1 as i2c
 *input ,set 0 not as i2c input A4:5b7[1] set 1 as spi input,set 0 not as spi
 *input ;5b7[5] set 1 as i2c input ,set 0 not as i3c input B6:5b7[2] set 1 as
 *spi input,set 0 not as spi input ;5b7[6] set 1 as i2c input ,set 0 not as i4c
 *input D7:5b7[3] set 1 as spi input,set 0 not as spi input ;5b7[7] set 1 as i2c
 *input ,set 0 not as i5c input
 */
...
void i2c_master_init(unsigned char SlaveID, unsigned char DivClock) {
  ...
  reg_spi_sp &= ~FLD_SPI_ENABLE; // force PADs act as I2C; i2c and spi share the
                                 // hardware of IC
}
...
void i2c_slave_init(unsigned char device_ID, I2C_SlaveMode mode,
  ...
  reg_i2c_mode &=
      (~FLD_I2C_MASTER_EN); // enable slave mode
                            // notice that: both dma and mapping mode need this
                            // to trigger data address auto increase(confirmed
                            // by congqing and sihui)
  reg_i2c_mode |= FLD_I2C_ADDR_AUTO_ADD;

  if (mode == I2C_SLAVE_MAP) {
    reg_i2c_mode |= FLD_I2C_SLAVE_MAPPING;
    reg_i2c_slave_map_addrl =
        (unsigned char)(((unsigned int)pMapBuf & 0xff)); //
...
void i2c_write_byte(unsigned int Addr, unsigned int AddrLen,
                    unsigned char Data) {
  reg_i2c_id &=
      (~FLD_I2C_WRITE_READ_BIT); // SlaveID & 0xfe,.i.e write data. R:High W:Low
  if (AddrLen == 0) { // telink 82xx slave mapping mode no need send any address
    ...
    // lanuch start /id    start
    reg_i2c_ctrl =
        (FLD_I2C_CMD_ID | FLD_I2C_CMD_START); // when addr=0,other (not telink)
                                              // iic device no need start signal
#endif
  }
  if (AddrLen == 1) {
    ...
    reg_i2c_ctrl = (FLD_I2C_CMD_ID | FLD_I2C_CMD_ADDR | FLD_I2C_CMD_DO |
                    FLD_I2C_CMD_START);
  } else if (AddrLen == 3) {
    ...
    reg_i2c_ctrl = (FLD_I2C_CMD_ID | FLD_I2C_CMD_ADDR | FLD_I2C_CMD_DO |
                    FLD_I2C_CMD_DI | FLD_I2C_CMD_START);
  }
...
unsigned char i2c_read_byte(unsigned int Addr, unsigned int AddrLen) {
  ...
  // start + id(Read)
  reg_i2c_id |= FLD_I2C_WRITE_READ_BIT; // SlaveID & 0xfe,.i.e write data.
                                        // Read:High  Write:Low
...
void i2c_write_series(unsigned int Addr, unsigned int AddrLen,
  ...
  if (AddrLen == 0) { // telink 82xx slave mapping mode no need send any address
    ...
    reg_i2c_ctrl =
        (FLD_I2C_CMD_ID |
         FLD_I2C_CMD_START); // when addr=0,other (not telink) iic device no
                             // need start signal
#endif
    // while(reg_i2c_status & FLD_I2C_CMD_BUSY	);
  } else if (AddrLen == 1) {
