...
#include "timer.h"
/**
 * @brief       This function servers to perform aes_128 encryption for 16-Byte
 * input data with specific 16-Byte key
 ...
 *              not 0: failure
 */
int aes_encrypt(unsigned char *Key, unsigned char *Data,
...
/**
 * @brief       This function servers to perform aes_128 decryption for 16-Byte
 * input data with specific 16-Byte key
 * @param[in]   Key the pointer to the 16-Byte Key
 ...
 *              not 0: failure
 */
int aes_decrypt(unsigned char *Key, unsigned char *Data,
                unsigned char *Result) {
  unsigned int tmp = 0;
  unsigned char *p = Data;
  unsigned char i = 0;
 ...
 *              not 0: failure
 */
int aes_dma_encrypt(unsigned char *Key, unsigned long *Data,
                    unsigned short DataSize, unsigned long *Result,
                    unsigned short ResultSize) {
  write_reg8(0xc10,
             ((unsigned long)Result) & 0xff); // set memory address low byte
  write_reg8(0xc11, (((unsigned long)Result) >> 8) &
                        0xff); // set memory address high byte
  reg_dma4_addrHi = 0x04;
  reg_dma4_size = ResultSize / 16; // set memory size
  reg_dma4_mode = 0x01;

  write_reg8(0xc14, ((unsigned long)Data) & 0xff); // set memory address low
                                                   // byte
  write_reg8(0xc15, (((unsigned long)Data) >> 8) &
                        0xff); // set memory address high byte
 ...
 *              not 0: failure
 */
int aes_dma_decrypt(unsigned char *Key, unsigned long *Data,
                    unsigned short DataSize, unsigned long *Result,
                    unsigned short ResultSize) {
  write_reg8(0xc10,
             ((unsigned long)Result) & 0xff); // set memory address low byte
