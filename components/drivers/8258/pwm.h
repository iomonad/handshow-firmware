...
typedef enum {
  PWM0_PULSE_NORMAL = 0, // duty cycle and period from TCMP0/TMAX0
                         // 0x794~0x797
  PWM0_PULSE_SHADOW = BIT(
      14), // duty cycle and period from TCMP0_SHADOW / TMAX0_SHADOW 0x7c4~0x7c7
} Pwm0Pulse_SelectDef;
 ...
 * @return	  none.
 */
static inline void pwm_set_cycle_and_duty(pwm_id id, unsigned short cycle_tick,
 ...
 * @return	  none.
 */
static inline void pwm_set_pwm0_shadow_cycle_and_duty(unsigned short cycle_tick,
                                                      unsigned short cmp_tick) {
 ...
 * @return	  none.
 */
static inline void pwm_revert(pwm_id id) { reg_pwm_invert |= BIT(id); }
 ...
 * @return	  none.
 */
static inline void pwm_n_revert(pwm_id id) { reg_pwm_n_invert |= BIT(id); }
...
/**
 * @brief     This fuction serves to set trigger level of interrupt for IR FiFo
 * mode
...
/**
 * @brief     This fuction serves to clear data in fifo. Only when pwm is in not
 * active mode, it is possible to clear data in fifo.
 * @param[in] none
 * @return	  none
 */
 ...
 * @param[in] use_shadow - determine whether the configuration of shadow cmp and
 * shadow max is used 1: use shadow, 0: not use
 * @param[in] carrier_en - enable sending carrier, 1: enable, 0: disable
 * @return	  none
 */
static inline void pwm_ir_fifo_set_data_entry(unsigned short pulse_num,
                                              unsigned char use_shadow,
                                              unsigned char carrier_en) {
  static unsigned char index = 0;
  unsigned short cfg_data =
      pulse_num + ((use_shadow & BIT(0)) << 14) + ((carrier_en & BIT(0)) << 15);
...
pwm_config_dma_fifo_waveform(int carrier_en, Pwm0Pulse_SelectDef pulse_type,
