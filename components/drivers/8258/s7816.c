 ...
 * 				7816clk = sysclk/(0x7b[6:4]*2),	0x7b[7]:enable
 * 7816clk 7816clk:  0x40-2Mhz   0x20-4Mhz baudrate: 0x40-16194  0x20-32388
 * @return     	none
 */
...
/**
 * @brief      	This function is used to set address and size of buffer 7816
 * module of MCU
 * @param[in]  	*RecvAddr		-set the address of buffer to receive
 * data
 * @param[in]  	RecvBufLen		-set the length of buffer to receive
 * data
 * @return     	none
 */
...
/**
 * @brief      	This function is used to send data to ID card,after succeeding
 * in getting ATR
...
unsigned char s7816_dma_send(unsigned char *TransAddr) {
  ...
  if (reg_uart_status1 &
      FLD_UART_TX_DONE) { // waiting for finishing an operation of sending
 ...
 * @param[in]  	mode	- half_duplex_mode_TX/RX is transformed by setting
 * 0x9b[5] 0x9b[5]=1:half_duplex_mode_RX;0x9b[5]=0:half_duplex_mode_TX
 * @return     	none
 */
