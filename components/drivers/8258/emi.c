...
static unsigned char emi_zigbee_tx_packet[48]
    __attribute__((aligned(4))) = {19, 0, 0, 0, 20, 0, 0};
static unsigned char emi_ble_tx_packet[48]
    __attribute__((aligned(4))) = {39, 0, 0, 0, 0, 37};
...
void rf_emi_rx(RF_ModeTypeDef mode, signed char rf_chn) {
  write_reg32(0x408, 0x29417671); // accesscode: 1001-0100 1000-0010 0110-1110
                                  // 1000-1110   29 41 76 71
 ...
 */
unsigned int rf_emi_get_rxpkt_cnt(void) { return emi_rx_cnt; }
 ...
 */
char rf_emi_get_rssi_avg(void) { return rssi; }
 ...
 */
unsigned char *rf_emi_get_rxpkt(void) { return emi_rx_packet; }
...
void rf_emi_tx_continue_setup(RF_ModeTypeDef rf_mode,
                              RF_PowerTypeDef power_level, signed char rf_chn,
...
/**
 * @brief   This function serves to generate random packets that need to be sent
 * in burst mode
 ...
 * @return  none.
 */
void rf_emi_tx_burst_setup(RF_ModeTypeDef rf_mode, RF_PowerTypeDef power_level,
                           signed char rf_chn, unsigned char pkt_type) {
  // #if 0
  unsigned char i;
  ...
  rf_set_power_level_index(
      (unsigned char)rf_power_Level_list[power_level & 0x3f]);
  if (pkt_type == 1)
    tx_data = 0x0f;
  else if (pkt_type == 2)
    tx_data = 0x55;
...
/**
 * @brief   This function serves to send packets in the burst mode with PA ramp
 * up/down.
 * @param   rf_mode - mode of RF.
 * @param 	pkt_type - The type of data sent
 * 					   0:random data
...
void rf_emi_tx_burst_loop_ramp(RF_ModeTypeDef rf_mode, unsigned char pkt_type) {
  ...
  if ((rf_mode == RF_MODE_BLE_1M_NO_PN) || (rf_mode == RF_MODE_BLE_2M) ||
      (rf_mode == RF_MODE_LR_S8_125K) || (rf_mode == RF_MODE_LR_S2_500K)) // ble
  {
...
void rf_set_channel_singletone(signed char chn) // general
{
  ...
  write_reg8(0x1244,
             ((chnl_freq & 0x7f) << 1) | 1); // CHNL_FREQ_DIRECT   CHNL_FREQ_L
  write_reg8(0x1245, ((read_reg8(0x1245) & 0xc0)) |
                         ((chnl_freq >> 7) & 0x3f));             // CHNL_FREQ_H
