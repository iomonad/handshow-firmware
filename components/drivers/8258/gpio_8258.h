 ...
 * @param[in]  en  -  if mcu wake up from deep retention mode, it is NOT
 * necessary to reset analog register
 * @return     none.
 */
 ...
 * @param[in]  value - enable or disable the pin's output function(0: disable,
 * 1: enable)
 * @return     none
 */
...
/**
 * @brief      This function determines whether the output function of a pin is
 * enabled.
 * @param[in]  pin - the pin needs to determine whether its output function is
 * enabled.
...
/**
 * @brief     This function determines whether the input function of a pin is
 * enabled.
 * @param[in] pin - the pin needs to determine whether its input function is
 * enabled.
 ...
 */
static inline unsigned int gpio_read_cache(GPIO_PinTypeDef pin,
                                           unsigned char *p) {
 ...
 * @param[in] falling - value of the edge polarity(1: falling edge 0: rising
 * edge)
 * @return    none
 */
static inline void gpio_set_interrupt_pol(GPIO_PinTypeDef pin,
 ...
 * @return    none
 */
static inline void gpio_set_interrupt(GPIO_PinTypeDef pin,
                                      GPIO_PolTypeDef falling) {
  unsigned char bit = pin & 0xff;
 ...
 * @return    none
 */
static inline void gpio_en_interrupt(GPIO_PinTypeDef pin,
                                     int en) // reg_irq_mask: FLD_IRQ_GPIO_EN
{
  unsigned char bit = pin & 0xff;
  if (en) {
    BM_SET(reg_gpio_irq_wakeup_en(pin), bit);
 ...
 * @return    none
 */
static inline void gpio_set_interrupt_risc0(GPIO_PinTypeDef pin,
                                            GPIO_PolTypeDef falling) {
  unsigned char bit = pin & 0xff;
 ...
 */
static inline void
gpio_en_interrupt_risc0(GPIO_PinTypeDef pin,
                        int en) // reg_irq_mask: FLD_IRQ_GPIO_RISC0_EN
{
  unsigned char bit = pin & 0xff;
  if (en) {
    BM_SET(reg_gpio_irq_risc0_en(pin), bit);
 ...
 * @return    none
 */
static inline void gpio_set_interrupt_risc1(GPIO_PinTypeDef pin,
                                            GPIO_PolTypeDef falling) {
  unsigned char bit = pin & 0xff;
 ...
 */
static inline void
gpio_en_interrupt_risc1(GPIO_PinTypeDef pin,
                        int en) // reg_irq_mask: FLD_IRQ_GPIO_RISC1_EN
{
  unsigned char bit = pin & 0xff;
  if (en) {
    BM_SET(reg_gpio_irq_risc1_en(pin), bit);
 ...
 * @param[in]  En - enables or disables the internal pull-up resistor(1: enable
 * 0: disable)
 * @return     none
 */
...
/**
 * @brief      This function serves to set GPIO MUX function as DP and DM pin of
 * USB
 * @param[in]  none.
 * @return     none.
 */
