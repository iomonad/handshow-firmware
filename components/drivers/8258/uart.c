...
/**
 * @brief     This function is used to look for the prime.if the prime is
 * finded,it will return 1, or return 0.
...
/**
 * @brief  		This funciton serves to calculate the best bwpc(bit
 * width) .i.e reg0x96
...
static void GetBetterBwpc(unsigned int baut_rate, unsigned int tmp_sysclk) {
  ...
  for (i = 3; i <= 15; i++) {
    ...
    D_dec[i - 3] = D_intdec[i - 3] -
                   10 * (D_intdec[i - 3] / 10); /// get the decimal section
 ...
 *
 *	  	  	  	16Mhz        9600          118
 *13 19200         118     			 6 115200         9
 *13
 *
 *	  	  	  	24Mhz        9600          249 9 19200
 *124               9 115200         12    			 15
 *
 *	  	  	  	32Mhz        9600          235 13 19200
 *235               6 115200         17    			 13
 *
 *	  	  	  	48Mhz        9600          499 9 19200
 *249               9 115200         25    			 15
 *
 ...
 * @return     none
 */
void uart_init(unsigned short g_uart_div, unsigned char g_bwpc,
               UART_ParityTypeDef Parity, UART_StopBitTypeDef StopBit) {
  ...
  reg_uart_rx_timeout1 =
      UART_BW_MUL2; // if over 2*(tmp_bwpc+1),one transaction end.
 ...
 * @return     none
 */
void uart_init_baudrate(unsigned int Baudrate, unsigned int System_clock,
 ...
 * 0x9d ie 0x9d[3]. If the cnt register value(0x9c[0,3]) larger or equal than
 * the value of 0x99[0,3] or the cnt register value(0x9c[4,7]) less or equal
 * than the value of 0x99[4,7], it will set the irq bit of status register 0x9d,
 * ie 0x9d[3]
 ...
 *            variable uart_TxIndex,it must cycle the four registers 0x90 0x91
 * 0x92 0x93 for the design of SOC. so we need variable to remember the index.
...
/**
 * @brief     uart send data function, this  function tell the DMA to get data
 * from the RAM and start the DMA transmission
...
volatile unsigned char uart_dma_send(unsigned char *Addr) {
  if (reg_uart_status1 & FLD_UART_TX_DONE) {
    reg_dma1_addr = (unsigned short)((
        unsigned int)Addr); // packet data, start address is sendBuff+1
...
/**
 * @brief     data receive buffer initiate function. DMA would move received
 * uart data to the address space, uart packet length needs to be no larger than
 * (recBuffLen - 4).
...
/**
 * @brief     This function determines whether parity error occurs once a packet
 * arrives.
 * @param[in] none
 ...
 * @param[in] Thresh - threshold of trig RTS pin's level toggle(only for auto
 * mode), it means the number of bytes that has arrived in Rx buf.
 * @param[in] Invert - whether invert the output of RTS pin(only for auto mode)
 * @param[in] pin   - RTS pin select,it can be
 * GPIO_PA4/GPIO_PB3/GPIO_PB6/GPIO_PC0.
 * @return    none
 */

void uart_set_rts(unsigned char Enable, UART_RTSModeTypeDef Mode,
                  unsigned char Thresh, unsigned char Invert,
                  UART_RtsPinDef pin) {
  if (Enable) {
 ...
 * @param[in]  pin   - CTS pin select,it can be
 * GPIO_PA3/GPIO_PB2/GPIO_PC4/GPIO_PD1.
 * @return     none
 */
void uart_set_cts(unsigned char Enable, unsigned char Select,
                  UART_CtsPinDef pin) {
  if (Enable) {
...
void uart_gpio_set(UART_TxPinDef tx_pin, UART_RxPinDef rx_pin) {
  // note: pullup setting must before uart gpio config, cause it will lead to
  // ERR data to uart RX buffer(confirmed by sihui&sunpeng) PM_PIN_PULLUP_1M
  // PM_PIN_PULLUP_10K
  gpio_setup_up_down_resistor(
      tx_pin, PM_PIN_PULLUP_10K); // must, for stability and prevent from
                                  // current leakage
  gpio_setup_up_down_resistor(
      rx_pin, PM_PIN_PULLUP_10K); // must  for stability and prevent from
                                  // current leakage

  gpio_set_func2(tx_pin, AS_UART); // set tx pin
  gpio_set_func(rx_pin, AS_UART);  // set rx pin

  gpio_set_input_en(tx_pin, 1); // experiment shows that tx_pin should open
                                // input en(confirmed by qiuwei)
