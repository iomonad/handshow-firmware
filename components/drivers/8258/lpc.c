 ...
 * @return none
 */
void lpc_power_down(void) { analog_write(0x07, analog_read(0x07) | 0x08); }
/**
 * @brief This function provides power for low power comparator.
 * @param[in] none
 * @return none
 */
void lpc_power_on(void) { analog_write(0x07, analog_read(0x07) & 0xf7); }
/**
 * @brief This function selects input channel for low power comparator .
 * @param[in] pin-selected input channel.Input derived from external
 * PortB(PB<1>~PB<7>).
 * @return none
 */
 ...
 * .
 * @param[in] mode-lower power comparator working mode includes normal mode and
 * low power mode.
