...
#define RF_BLE_PACKET_LENGTH_OK(p)                                             \
  (*((unsigned int *)p) == p[5] + 13) // dma_len must 4 byte aligned
...
#define RF_ZIGBEE_PACKET_TIMESTAMP_GET(p)                                      \
  (p[p[0] - 4] | (p[p[0] - 3] << 8) | (p[p[0] - 2] << 16) | (p[p[0] - 1] << 24))
...
#define RF_NRF_SB_PACKET_CRC_GET(p)                                            \
  ((p[p[0] - 8] << 8) +                                                        \
   p[p[0] - 7]) // Note: here assume that the MSByte of CRC is received first
#define RF_NRF_SB_PACKET_RSSI_GET(p) (p[p[0] + 2])
#define RF_NRF_ESB_PACKET_TIMESTAMP_GET(p)                                     \
  (p[p[0] - 4] | (p[p[0] - 3] << 8) | (p[p[0] - 2] << 16) | (p[p[0] - 1] << 24))
#define RF_NRF_SB_PACKET_TIMESTAMP_GET(p)                                      \
  (p[p[0] - 4] | (p[p[0] - 3] << 8) | (p[p[0] - 2] << 16) | (p[p[0] - 1] << 24))
 ...
 *							If you set a value
 *outside the range, you will be set to fail.
 ...
 *										Note:
 *The effect for 3-byte access_code is not good.
 *
 *	@return	 	none
 */
...
static inline void rf_access_code_comm(unsigned int acc) {
  ...
  write_reg8(0x800405,
             read_reg8(0x405) |
                 0x80); // setting acess code needs to writ 0x405 access code
                        // trigger bit 1 to enable in long range mode,,and the
                        // mode of  BLE1M and BLE2M need not.
}
...
static inline void rf_longrange_access_code_comm(unsigned int acc) {
  ...
  write_reg8(0x800405,
             read_reg8(0x405) |
                 0x80); // setting acess code needs to writ 0x405 access code
                        // trigger bit to enable in long range mode,,and the
                        // mode of  BLE1M and BLE2M need not.
}
...
/**
 *	@brief		this function is to enable/disable each access_code
 *channel for RF Rx terminal.
 *	@param[in]	pipe  	Bit0~bit5 correspond to channel 0~5,
 *respectively. 0£ºDisable 1£ºEnable If ¡°enable¡± is set as 0x3f (i.e.
 *00111111), all access_code channels (0~5) are enabled.
 *	@return	 	none
 */
...
/**
 *	@brief		this function is to select access_code channel for RF Rx
 *terminal.
 *	@param[in]	pipe  	Bit0~bit5 correspond to channel 0~5,
 *respectively. 0£ºDisable 1£ºEnable If ¡°enable¡± is set as 0x3f (i.e.
 *00111111), all access_code channels (0~5) are enabled.
 ...
 * @return  none.
 */
static inline void rf_set_tx_rx_off_auto_mode(void) { write_reg8(0xf00, 0x80); }
/**
...
/**
 *	@brief	  	This function serves to determine whether sending a
 *packet of data is finished
 *	@param[in]	none.
 *	@return	 	Yes: 1, NO: 0.
 */
 ...
 *				After all packet data are sent, corresponding Tx
 *finish flag bit will be set as 1.By reading this flag bit, it can check
 *whether packet transmission is finished. After the check, it¡¯s needed to
 *				manually clear this flag bit so as to avoid
 *misjudgment.
 *   @param      none
 *	@return	 	none
 */
...
/**
 *	@brief	  	This function serves to determine whether sending a
 *packet of rf_tx_acc_code_select is finished
 *	@param[in]	none.
 *	@return	 	Yes: 1, NO: 0.
 */
 ...
 *
 *	@param[out]	RF_RxAddr  	Pointer for Rx buffer in RAM(Generally
 *it¡¯s starting address of an array.Should be 4-byte aligned)
 *	@param[in]	size   		Rx buffer size (It¡¯s an integral
 *multiple of 16)
 *	@param[in]	PingpongEn 	Enable/Disable Ping-Pong buffer
 *1£ºEnable 0£ºDisable Note: When ¡°PingpongEn¡± is set as 0, received RF data
 *will be stored in RAM pointed by ¡° RF_RxAddr¡±. When ¡°PingpongEn¡± is set as
 *1, received RF data will be stored in buffer0 and buffer1 successively. The
 *RAM size reserved for received RF data should be double of ¡°Size¡±.
 *
 *	@return	 	none
 */

extern void rf_rx_buffer_set(unsigned char *RF_RxAddr, int size,
...
/**
 *	@brief	  	This function serves to determine whether a packet of
 *data received is right
 *	@param[in]	none.
 *	@return	 	Yes: 1, NO: 0.
 */
 ...
 *				After all packet data are resive, corresponding
 *Rx finish flag bit will be set as 1.By reading this flag bit, it can check
 *whether packet transmission is finished. After the check, it¡¯s needed to
 *				manually clear this flag bit so as to avoid
 *misjudgment.
 *   @param      none
 ...
 *	@param[in]	addr   Tx packet address in RAM. Should be 4-byte
 *aligned.
...
/**
 *	@brief	  	This function serves to start Rx of auto mode. In this
 *mode, RF module stays in Rx status until a packet is received or it fails to
 *receive packet when timeout expires. Timeout duration is set by the parameter
 *"tick". The address to store received data is set by the function ¡°addr¡±.
 *	@param[in]	addr - The address to store received data.
 *	@param[in]	tick - Unit is us. It indicates timeout duration in Rx
 *status.Max value: 0xffffff (16777215)
 *	@return	 	none
 */
 ...
 *				In this mode, a packet is sent first,RF module
 *waits for 10us, stays in Rx status until data is received or timeout expires,
 *				then exits this mode.Timeout duration is set by
 *the parameter ¡°timeout_us¡±.The address to store received data is set by the
 ...
 *				In this mode,RF module stays in Rx status until
 *a packet is received or it fails to receive packetwhen timeout expires. If a
 *packet is received within the timeout duration, RF module will wait for
 *10us,send a packet, and then exit this mode. If it fails to receive packet
 *when timeout expires, RF module will directly exit this mode.Timeout duration
 *is set by the parameter "timeout_us".	The address to store received data is
 *set by the function ¡°RF_RxBufferSet¡±.
 *
 *	@param[in]	addr 	Tx packet address in RAM. Should be 4-byte
 *aligned.
 *	@param[in]	tick   Tick value of system timer. It determines when to
 ...
 *mode.
 *	@param[in]	timeout_us  Unit is us. It indicates timeout duration in
 *Rx status. Max value: 0xffffff (16777215)
 *
 *	@return	 	none
 */
 ...
 * @return    none.
 */
static inline void rf_ble_tx_done() { write_reg8(0x800f02, 0x45); }
 ...
 * @return  none
 */
static inline void reset_sn_nesn(void) { REG_ADDR8(0xf01) = 0x01; }
 ...
 * @return  none.
 */
static inline void rf_set_ble_crc_adv() { write_reg32(0x800424, 0x555555); }
...
static inline unsigned char is_rf_receiving_pkt(void) {
  // if the value of [5:4] of the reg_0x448 is 0b10 or 0b11, it means that the
  // RF is in the receiving packet phase.(confirmed by junwen)
 ...
 *	@return	 	init_val 		- CRC
 */
extern unsigned short crc16_ccitt_cal(unsigned char *input, unsigned int len,
...
extern void
rf_tx_500k_simulate_100k(unsigned char *preamble, unsigned char preamble_len,
...
/**
 *	@brief	  	This function is to start energy detect of the current
 *channel for zigbee mode Before using it.
 *   @param      none
 *	@return	 	none
 */
...
/**
 *	@brief	  	This function is to stop energy detect and get energy
 *detect value of the current channel for zigbee mode.
 *   @param      none
...
/**
 * @brief      This function process the received packet in 1mbps shockburst
 mode only for hanshow for the
 *             compatiblity with third-party chips. The process includes
 data-whitening transformation and crc check.
 * @param[in]  rx_buf - the rf rx buffer containing the received packet(dma
 length+payload+3 byte crc)
 * @param[in]  len - the expected rx length of shockburst mode, containing
 payload and 3byte crc
 * @return     the status of the processing procesure. 1: the received packet is
 correct, 0: the received packet is incorrect
 */

unsigned char rx_packet_process_1mbps(unsigned char *rx_buf, unsigned int len);
/**
 * @brief      This function process the tx packet in 1mbps shockburst mode only
 for hanshow for the
 *             compatiblity with third-party chips. The process includes
 data-whitening transformation and crc padding.
 * @param[in]  tx_buf - the rf tx buffer containing the tx packet(dma
 length+payload+3 byte crc)
 * @param[in]  len - the expected tx length of shockburst mode, containing
 payload and 3byte crc
 * @return     none
 */
void tx_packet_process_1mbps(unsigned char *tx_buf, unsigned char *payload,
                             unsigned int len);
