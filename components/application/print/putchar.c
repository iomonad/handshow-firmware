...
#define USB_EP_IN                                                              \
  (USB_EDP_PRINTER_IN &                                                        \
   0X07) //  from the point of HOST 's view,  IN is the printf out
...
int swire_putc(int c) {
  ...
  int i = 0;
  while (i++ < USB_PRINT_TIMEOUT) {
    if (reg_usb_ep_ptr(USB_EP_IN) - reg_usb_ep_ptr(USB_EP_OUT) <=
        USB_SWIRE_BUFF_SIZE) { //  not full
      reg_usb_ep_dat(USB_EP_IN) = (unsigned char)c;
      return c;
    }
  }
...
_attribute_ram_code_ int uart_putc(char byte) // GPIO simulate uart print func
{
  ...
  // REG_ADDR8(0x582+((DEBUG_INFO_TX_PIN>>8)<<3)) &= ~(DEBUG_INFO_TX_PIN & 0xff)
  // ;//Enable output

  unsigned int pcTxReg =
      (0x583 + ((DEBUG_INFO_TX_PIN >> 8) << 3)); // register GPIO output
