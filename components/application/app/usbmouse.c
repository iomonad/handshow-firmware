...
void usbmouse_add_frame(rf_packet_mouse_t *packet_mouse) {
  ...
  for (u8 i = 0; i < new_data_num; i++) {
    memcpy4((int *)(&mouse_dat_buff[usbmouse_wptr]),
            (int *)(&packet_mouse->data[i * sizeof(mouse_data_t)]),
            sizeof(mouse_data_t));
...
void usbmouse_release_check() {
  if (usbmouse_not_released &&
      clock_time_exceed(usbmouse_data_report_time, USB_MOUSE_RELEASE_TIMEOUT)) {
    u32 release_data = 0;

    if (usbmouse_hid_report(USB_HID_MOUSE, (u8 *)(&release_data),
                            MOUSE_REPORT_DATA_LEN)) {
...
void usbmouse_report_frame() {
  ...
  if (usbmouse_wptr != usbmouse_rptr) {
    ...
    int ret = usbmouse_hid_report(USB_HID_MOUSE, (u8 *)(&data),
                                  MOUSE_REPORT_DATA_LEN);
    if (ret) {
      BOUND_INC_POW2(usbmouse_rptr, USBMOUSE_BUFF_DATA_NUM);
    }
...
int usbmouse_hid_report(u8 report_id, u8 *data, int cnt) {
  ...
  reg_usb_ep_ctrl(USB_EDP_MOUSE) =
      FLD_EP_DAT_ACK |
      (edp_toggle[USB_EDP_MOUSE] ? FLD_USB_EP_DAT1 : FLD_USB_EP_DAT0); // ACK
