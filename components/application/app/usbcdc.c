...
void CDC_Device_ProcessControlRequest(u8 bRequest, u16 wValue, u16 wIndex,
                                      u16 wLength) {
...
void usbcdc_setRxBuf(u8 *buf) { cdc_v->rxBuf = buf; }
...
u8 usbcdc_sendBulkData(void) {
  ...
  len =
      (cdc_v->lenToSend > CDC_TXRX_EPSIZE) ? CDC_TXRX_EPSIZE : cdc_v->lenToSend;
  cdc_v->lenToSend -= len;
...
u8 usbcdc_isAvailable(void) { return (cdc_v->txBuf == NULL); }
