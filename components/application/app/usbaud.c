...
void usbaud_hid_report(char format, char volume) {
	...
	WriteEndPoint(EDP_ID_AUDIO_HID, sAudioHidReport, sizeof(sAudioHidReport));
     ...
     USB_MIC_ENABLE) //  use for volumn control, mute, next, prev track,  move
                     //  to mouse hid
...
u8 usbaud_speaker_vol_get(void) {
  // return ((g_usb_audio_status.speaker_mute << 7) |
  // g_usb_audio_status.speaker_vol);
...
void usbaud_mic_en(int en) { mic_setting.mute = en; }
...
int usb_audio_class_out_intf_proc(u8 type, u8 feature_id) {
  ...
  case 0x02:
    ...
    case AUDIO_FEATURE_ID_SPEAKER: // Feature ID 0x02, Speaker
      p_aud->speaker_vol =
          usbhw_read_ctrl_ep_data() + (usbhw_read_ctrl_ep_data() << 8);
      p_aud->speaker_vol =
          ((p_aud->speaker_vol - (short)SPEAKER_VOL_MIN + SPEAKER_VOL_STEP) *
           (10 - 1)) >>
          12;
