...
#endif
#if (0)
  USB_INTF_AUDIO_HID, //  use for volumn control, mute, next, prev track,  move
                      //  to mouse hid
#endif
...
#if (USB_MIC_ENABLE)
#define USB_MIC_CHANNELS_LEN                                                   \
  (MIC_CHANNLE_COUNT * (MIC_SAMPLE_RATE * MIC_RESOLUTION_BIT / 1000 / 8))
#endif
...
typedef struct {
  u32 dwLength;   // length, in bytes, of the complete extended compat ID
                  // descriptor
  u16 bcdVersion; // BCD	The descriptor¡¯s version number, in binary
