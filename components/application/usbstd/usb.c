...
void usb_handle_std_intf_req() {
  u8 value_h = (control_request.Value >> 8) & 0xff;
#if (USB_MIC_ENABLE || USB_SPEAKER_ENABLE || USB_MOUSE_ENABLE ||               \
     USB_KEYBOARD_ENABLE || USB_SOMATIC_ENABLE)
...
#endif
#if (USB_KEYBOARD_ENABLE)
    if (index_l == (USB_DESCRIPTER_CONFIGURATION_FOR_KM_DONGLE
                        ? keyboard_interface_number
                        : USB_INTF_KEYBOARD)) {
      // keyboard
...
#endif
#if (USB_MOUSE_ENABLE)
    else if (index_l == (USB_DESCRIPTER_CONFIGURATION_FOR_KM_DONGLE
  ...
  case HID_REQ_SetReport:
    ...
    case HID_REPORT_ITEM_Feature:
...
#if (USB_SET_REPORT_FEATURE_SUPPORT)
      {
        ...
        ev_emit_event_syn(EV_USB_SET_REPORT,
                          (void *)(&rpt)); // send in report id
      }
#endif
      break;
        ...
        if (custom_reg_cmd) {
          ...
          if ((host_cmd[1] & 0x0c) == 0) { // write core register
            if (len == 0) {
              for (int k = 0; k < 4; k++) {
                custom_read_dat =
                    (custom_read_dat >> 8) | (read_reg8(adr++) << 24);
              }
            } else if (len == 1) {
  ...
  case HID_REQ_GetReport:
    ...
    if (control_request.Value == 0x0305) {
      if (USB_CUSTOM_HID_REPORT_REG_ACCESS && custom_reg_cmd) {
        ...
        usbhw_write_ctrl_ep_data(0x00);
        usbhw_write_ctrl_ep_data(host_cmd_paring_ok ? 0xa1
                                                    : 0x00); // For binding OK
        usbhw_write_ctrl_ep_data(0x00);
        usbhw_write_ctrl_ep_data(0x00);
...
void usb_handle_set_intf() {
  ...
  if (USB_INTF_MIC == intf_index && value_l) {
    ...
    //		reg_usb_ep_ctrl(USB_EDP_MIC) = (MIC_CHANNLE_COUNT == 2 ? 0x81 :
    //0xc1);
...
int usb_suspend_check(void) { return 0; }
