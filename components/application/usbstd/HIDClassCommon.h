...
#define HID_DESCRIPTOR_JOYSTICK(NumAxis, MinAxisVal, MaxAxisVal,               \
                                MinPhysicalVal, MaxPhysicalVal, Buttons)       \
  HID_RPT_USAGE_PAGE(8, 0x01), HID_RPT_USAGE(8, 0x04),                         \
      HID_RPT_COLLECTION(8, 0x01), HID_RPT_USAGE(8, 0x01),                     \
      HID_RPT_COLLECTION(8, 0x00), HID_RPT_USAGE_MINIMUM(8, 0x30),             \
      ...
      HID_RPT_REPORT_SIZE(                                                     \
          8, ((((MinAxisVal >= -0xFF) && (MaxAxisVal <= 0xFF)) ? 8 : 16))),    \
      HID_RPT_INPUT(8, HID_IOF_DATA | HID_IOF_VARIABLE | HID_IOF_ABSOLUTE),    \
      HID_RPT_END_COLLECTION(0), HID_RPT_USAGE_PAGE(8, 0x09),                  \
      HID_RPT_USAGE_MINIMUM(8, 0x01), HID_RPT_USAGE_MAXIMUM(8, Buttons),       \
      HID_RPT_LOGICAL_MINIMUM(8, 0x00), HID_RPT_LOGICAL_MAXIMUM(8, 0x01),      \
      HID_RPT_REPORT_SIZE(8, 0x01), HID_RPT_REPORT_COUNT(8, Buttons),          \
      HID_RPT_INPUT(8, HID_IOF_DATA | HID_IOF_VARIABLE | HID_IOF_ABSOLUTE),    \
      HID_RPT_REPORT_SIZE(8, (8 - (Buttons % 8))),                             \
      HID_RPT_REPORT_COUNT(8, 0x01), HID_RPT_INPUT(8, HID_IOF_CONSTANT),       \
      HID_RPT_END_COLLECTION(0)

#define HID_DESCRIPTOR_KEYBOARD(MaxKeys)                                       \
  HID_RPT_USAGE_PAGE(8, 0x01), HID_RPT_USAGE(8, 0x06),                         \
      HID_RPT_COLLECTION(8, 0x01), HID_RPT_USAGE_PAGE(8, 0x07),                \
      HID_RPT_USAGE_MINIMUM(8, 0xE0), HID_RPT_USAGE_MAXIMUM(8, 0xE7),          \
      HID_RPT_LOGICAL_MINIMUM(8, 0x00), HID_RPT_LOGICAL_MAXIMUM(8, 0x01),      \
      HID_RPT_REPORT_COUNT(8, 0x08), HID_RPT_REPORT_SIZE(8, 0x01),             \
      HID_RPT_INPUT(8, HID_IOF_DATA | HID_IOF_VARIABLE | HID_IOF_ABSOLUTE),    \
      HID_RPT_INPUT(8, HID_IOF_CONSTANT | HID_IOF_VARIABLE),                   \
      HID_RPT_REPORT_COUNT(8, 0x05), HID_RPT_USAGE_PAGE(8, 0x08),              \
      HID_RPT_USAGE_MINIMUM(8, 0x01), HID_RPT_USAGE_MAXIMUM(8, 0x05),          \
      HID_RPT_OUTPUT(8, HID_IOF_DATA | HID_IOF_VARIABLE | HID_IOF_ABSOLUTE |   \
                            HID_IOF_NON_VOLATILE),                             \
      HID_RPT_REPORT_COUNT(8, 0x01), HID_RPT_REPORT_SIZE(8, 0x03),             \
      HID_RPT_OUTPUT(8, HID_IOF_CONSTANT), HID_RPT_REPORT_COUNT(8, 0x06),      \
      HID_RPT_REPORT_SIZE(8, 0x08), HID_RPT_LOGICAL_MINIMUM(8, 0x00), 0x26,    \
      0xa4, 0x00, HID_RPT_USAGE_PAGE(8, 0x07), HID_RPT_USAGE_MINIMUM(8, 0x00), \
      0x2a, 0xa4, 0x00,                                                        \
      HID_RPT_INPUT(8, HID_IOF_DATA | HID_IOF_ARRAY | HID_IOF_ABSOLUTE),       \
      HID_RPT_END_COLLECTION(0)

#define HID_DESCRIPTOR_MOUSE(MinAxisVal, MaxAxisVal, MinPhysicalVal,           \
                             MaxPhysicalVal, Buttons, AbsoluteCoords)          \
  HID_RPT_USAGE_PAGE(8, 0x01), HID_RPT_USAGE(8, 0x02),                         \
      HID_RPT_COLLECTION(8, 0x01), HID_RPT_USAGE(8, 0x01),                     \
      HID_RPT_COLLECTION(8, 0x00), HID_RPT_USAGE_PAGE(8, 0x09),                \
      HID_RPT_USAGE_MINIMUM(8, 0x01), HID_RPT_USAGE_MAXIMUM(8, Buttons),       \
      HID_RPT_LOGICAL_MINIMUM(8, 0x00), HID_RPT_LOGICAL_MAXIMUM(8, 0x01),      \
      HID_RPT_REPORT_COUNT(8, Buttons), HID_RPT_REPORT_SIZE(8, 0x01),          \
      HID_RPT_INPUT(8, HID_IOF_DATA | HID_IOF_VARIABLE | HID_IOF_ABSOLUTE),    \
      HID_RPT_REPORT_COUNT(8, 0x01),                                           \
      HID_RPT_REPORT_SIZE(8, (8 - (Buttons % 8))),                             \
      HID_RPT_INPUT(8, HID_IOF_CONSTANT), HID_RPT_USAGE_PAGE(8, 0x01),         \
      HID_RPT_USAGE(8, 0x30), HID_RPT_USAGE(8, 0x31),                          \
      HID_RPT_LOGICAL_MINIMUM(16, MinAxisVal),                                 \
      HID_RPT_LOGICAL_MAXIMUM(16, MaxAxisVal),                                 \
      HID_RPT_PHYSICAL_MINIMUM(16, MinPhysicalVal),                            \
      HID_RPT_PHYSICAL_MAXIMUM(16, MaxPhysicalVal),                            \
      ...
      HID_RPT_END_COLLECTION(0), HID_RPT_END_COLLECTION(0)

#define HID_DESCRIPTOR_VENDOR(VendorPageNum, CollectionUsage, DataINUsage,     \
                              DataOUTUsage, NumBytes)                          \
  HID_RPT_USAGE_PAGE(16, (0xFF00 | VendorPageNum)),                            \
      HID_RPT_USAGE(8, CollectionUsage), HID_RPT_COLLECTION(8, 0x01),          \
      HID_RPT_USAGE(8, DataINUsage), HID_RPT_LOGICAL_MINIMUM(8, 0x00),         \
      HID_RPT_LOGICAL_MAXIMUM(8, 0xFF), HID_RPT_REPORT_SIZE(8, 0x08),          \
