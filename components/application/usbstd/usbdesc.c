...
const USB_Descriptor_String_t language_desc = {
    {sizeof(USB_Descriptor_Hdr_t) + 2, DTYPE_String}, {LANGUAGE_ID_ENG}};
...
const USB_Descriptor_String_t vendor_desc = {
    {sizeof(USB_Descriptor_Hdr_t) + sizeof(STRING_VENDOR) - 2,
     DTYPE_String}, // Header
    STRING_VENDOR};
...
/** Serial number string. This is a Unicode string containing the device's
 * unique serial number, expressed as a series of uppercase hexadecimal digits.
 */
const USB_Descriptor_String_t serial_desc = {
    {sizeof(USB_Descriptor_Hdr_t) + sizeof(STRING_SERIAL) - 2,
     DTYPE_String}, // Header
    STRING_SERIAL};
...
const unsigned char OSFeatureDescriptor[] = {
    0x26, 0x01, 0x00, 0x00, 0x00, 0x01, 0x05, 0x00, 0x05, 0x00,

    // DeviceIdleEnabled
    0x36, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x24, 0x00,

    0x44, 0x00, 0x65, 0x00, 0x76, 0x00, 0x69, 0x00, 0x63, 0x00, 0x65, 0x00,
    0x49, 0x00, 0x64, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x45, 0x00, 0x6E, 0x00,
    0x61, 0x00, 0x62, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x64, 0x00, 0x00, 0x00,

    0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,

    // DefaultIdleState
    0x34, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x22, 0x00,

    0x44, 0x00, 0x65, 0x00, 0x66, 0x00, 0x61, 0x00, 0x75, 0x00, 0x6C, 0x00,
    0x74, 0x00, 0x49, 0x00, 0x64, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x53, 0x00,
    0x74, 0x00, 0x61, 0x00, 0x74, 0x00, 0x65, 0x00, 0x00, 0x00,

    0x04, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00,

    // default timeout value for selective suspend.
    0x38, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x26, 0x00,

    0x44, 0x00, 0x65, 0x00, 0x66, 0x00, 0x61, 0x00, 0x75, 0x00, 0x6C, 0x00,
    0x74, 0x00, 0x49, 0x00, 0x64, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x54, 0x00,
    0x69, 0x00, 0x6D, 0x00, 0x65, 0x00, 0x6F, 0x00, 0x75, 0x00, 0x74, 0x00,
    0x00, 0x00,

    0x04, 0x00, 0x00, 0x00, 0x88, 0x13, 0x00, 0x00,

    // user-enabled selective suspend.
    0x44, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x32, 0x00,

    0x55, 0x00, 0x73, 0x00, 0x65, 0x00, 0x72, 0x00, 0x53, 0x00, 0x65, 0x00,
    0x74, 0x00, 0x44, 0x00, 0x65, 0x00, 0x76, 0x00, 0x69, 0x00, 0x63, 0x00,
    0x65, 0x00, 0x49, 0x00, 0x64, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x45, 0x00,
    ...
    0x36, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00, 0x24, 0x00, 0x53, 0x00,
    0x79, 0x00, 0x73, 0x00, 0x74, 0x00, 0x65, 0x00, 0x6D, 0x00, 0x57, 0x00,
    0x61, 0x00, 0x6B, 0x00, 0x65, 0x00, 0x45, 0x00, 0x6E, 0x00, 0x61, 0x00,
    0x62, 0x00, 0x6C, 0x00, 0x65, 0x00, 0x64, 0x00, 0x00, 0x00,
...
#if USB_MOUSE_ENABLE
        {
            USB_INTF_MOUSE, 0x01, 'W',  'I',  'N',  'U',  'S',  'B',
...
#ifndef ID_PRODUCT
#define ID_PRODUCT                                                             \
  (ID_PRODUCT_BASE | (USB_PRINTER_ENABLE ? (1 << 0) : 0) |                     \
   (USB_SPEAKER_ENABLE ? (1 << 1) : 0) | (USB_MIC_ENABLE ? (1 << 2) : 0) |     \
   (USB_MOUSE_ENABLE ? (1 << 3) : 0) | (USB_KEYBOARD_ENABLE ? (1 << 4) : 0) |  \
   (USB_SOMATIC_ENABLE ? (1 << 5) : 0))
#endif
...
const USB_Descriptor_Device_t device_desc = {
    ...
    USB_CSCP_NoDeviceProtocol, // Protocol
#endif
    8, // Endpoint0Size, Maximum Packet Size for Zero Endpoint. Valid Sizes are
       // 8, 16, 32, 64
    ID_VENDOR, // VendorID
#if USB_CDC_ENABLE
    0x8846,
...
const USB_Descriptor_Configuration_t configuration_desc = {
...

    {
        ...
        // CDC_NotificationEndpoint =
        {
            ...
            (EP_TYPE_INTERRUPT | ENDPOINT_ATTR_NO_SYNC |
             ENDPOINT_USAGE_DATA),   // Attributes
    ...
    // printer_interface
    {
        ...
        NO_DESCRIPTOR                    // iInterface,  same as iProduct in
                      // USB_Descriptor_Device_t, or else not working
    },
    // printer_in_endpoint
    {
        {sizeof(USB_Descriptor_Endpoint_t),
         DTYPE_Endpoint},                     // length, bDescriptorType
    ...
    {sizeof(USB_Audio_StdDescriptor_FeatureUnit_t),
     ...
     USB_SPEAKER_FEATURE_UNIT_SOURCE_ID,
     1,                  // bControlSize
    ...
    {{sizeof(USB_Audio_Descriptor_InputTerminal_t), DTYPE_CSInterface},
     ...
     NO_DESCRIPTOR},
    // mic_feature_unit
    {{sizeof(USB_Audio_Descriptor_FeatureUnit_Mic_t), DTYPE_CSInterface},
     AUDIO_DSUBTYPE_CSInterface_Feature,
    ...
    {
        {sizeof(USB_Audio_Descriptor_Format_t) + sizeof(USB_Audio_SampleFreq_t),
         DTYPE_CSInterface},
    ...
    {
        {
            ...
            USB_EDP_SPEAKER,
            EP_TYPE_ISOCHRONOUS | (EP_SYNC_TYPE_ADAPTIVE << 2) |
                (EP_USAGE_TYPE_DATA << 4), // Attributes ENDPOINT_ATTR_ASYNC
    ...
    {
        {sizeof(USB_Audio_Descriptor_StreamEndpoint_Spc_t), DTYPE_CSEndpoint},
        AUDIO_DSUBTYPE_CSInterface_General,
    ...
    {
        {
            ...
            EP_TYPE_ISOCHRONOUS | (EP_SYNC_TYPE_SYNC << 2) |
                (EP_USAGE_TYPE_DATA << 4), // Attributes
            USB_MIC_CHANNELS_LEN,
            1 // PollingIntervalMS
        },
        0, // Refresh
        0  // SyncEndpointNumber
    ...
    {// audio_hid
     {
         ...
         HID_DTYPE_Report,
         {sizeof(usbaud_report_desc), 0x00}, // HIDReportLength sizeof(Report)
     },
     // audio_in_endpoint
     {
     ...
     NO_DESCRIPTOR},
    {
        // keyboard_hid
        {
            ...
            1, // TotalReportDescriptors
            HID_DTYPE_Report,
            {sizeof(keyboard_report_desc), 0x00}, // HIDReportLength
        },
    ...
    {// mouse_hid
     {
         ...
         1, // TotalReportDescriptors
         HID_DTYPE_Report,
         {sizeof(mouse_report_desc), 0x00}, // HIDReportLength
     },
     // mouse_in_endpoint
     {
         {sizeof(USB_Descriptor_Endpoint_t), DTYPE_Endpoint},
     ...
     NO_DESCRIPTOR},
    {
        // SOMATIC_hid
        {
            ...
            1, // TotalReportDescriptors
            HID_DTYPE_Report,
            {sizeof(somatic_report_desc), 0x00}, // HIDReportLength
        },
        // SOMATIC_in_endpoint
        {
            {sizeof(USB_Descriptor_Endpoint_t), DTYPE_Endpoint},
...
u8 *usbdesc_get_language(void) { return (u8 *)(&language_desc); }

u8 *usbdesc_get_vendor(void) { return (u8 *)(&vendor_desc); }

u8 *usbdesc_get_product(void) { return (u8 *)(&product_desc); }

#if (MS_OS_DESCRIPTOR_ENABLE)

u8 *usbdesc_get_OS_descriptor(void) { return (u8 *)(&microsoft_OS_desc); }
...
u8 *usbdesc_get_serial(void) { return (u8 *)(&serial_desc); }

u8 *usbdesc_get_device(void) { return (u8 *)(&device_desc); }

u8 *usbdesc_get_configuration(void) { return (u8 *)(&configuration_desc); }
...
#if (USB_CDC_ENABLE)
u8 *usbdesc_get_cdc(void) { return (u8 *)(&configuration_desc.cdc_descriptor); }
