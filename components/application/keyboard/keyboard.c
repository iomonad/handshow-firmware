...
typedef unsigned char
    kb_k_mp_t[ARRAY_SIZE(drive_pins)]; // typedef unsigned char kb_k_mp_t[8]

#if KB_MAP_DEFAULT

#ifndef KB_MAP_NORMAL
static const unsigned char kb_map_normal[ARRAY_SIZE(scan_pins)][ARRAY_SIZE(
    drive_pins)] = {
    {VK_PAUSE, VK_POWER, VK_EURO, VK_SLEEP, VK_RCTRL, VK_WAKEUP, VK_CTRL,
     VK_F5},
    ...
    {VK_P, VK_LBRACE, VK_SEMICOLON, VK_QUOTE, VK_BACKSLASH, VK_SLASH, VK_MINUS,
     VK_0},
    {VK_SCR_LOCK, VK_C9R1, VK_FN, VK_ALT, VK_MMODE, VK_RALT, VK_C9R6,
     VK_PRINTSCREEN},
    {VK_K14, VK_BACKSPACE, VK_BACKSLASH, VK_F11, VK_ENTER, VK_F12, VK_F9,
     VK_F10},
    {VK_HOME, VK_LEFT, VK_END, VK_SPACE, VK_NUM_LOCK, VK_DOWN, VK_DELETE,
     VK_POWER},
    {VK_UP, VK_NONE, VK_DOWN, VK_INSERT, VKPAD_SLASH, VK_RIGHT, VK_INSERT,
     VK_SLEEP},
    {VK_PAGE_UP, VK_RIGHT, VK_PAGE_DOWN, VKPAD_PERIOD, VKPAD_ASTERIX,
     VKPAD_MINUS, VK_PAGE_UP, VK_PAGE_DOWN},
    {VKPAD_PLUS, VK_K107, VKPAD_ENTER, VK_UP, VK_PLAY_PAUSE, VK_LEFT, VK_HOME,
     VK_END},
    {VK_WAKEUP, VK_SHIFT, VK_RSHIFT, VK_VOL_DN, VK_VOL_UP, VK_NEXT_TRK,
     VK_PREV_TRK, VK_MEDIA},
    {VK_MAIL, VK_WIN, VK_W_FORWRD, VK_W_STOP, VK_W_BACK, VK_W_REFRESH,
     VK_W_MUTE, VK_W_SRCH},
    {VK_KCL, VK_W_FAV, VK_RWIN, VK_MY_COMP, VK_STOP, VK_CAL, VK_WEB, VK_KCR},
};
#else
static const unsigned char kb_map_normal[ARRAY_SIZE(scan_pins)]
                                        [ARRAY_SIZE(drive_pins)] =
                                            KB_MAP_NORMAL;
#endif

#ifndef KB_MAP_NUM
static const unsigned char kb_map_num[ARRAY_SIZE(scan_pins)][ARRAY_SIZE(
    drive_pins)] = {
    {VK_PAUSE, VK_POWER, VK_EURO, VK_SLEEP, VK_RCTRL, VK_WAKEUP, VK_CTRL,
     VK_F5},
    {VK_Q, VK_TAB, VK_A, VK_ESC, VK_Z, VK_NCHG, VK_TILDE, VK_1},
    ...
    {VK_K14, VK_BACKSPACE, VK_BACKSLASH, VK_F11, VK_ENTER, VK_F12, VK_F9,
     VK_F10},
    {VKPAD_7, VKPAD_4, VKPAD_1, VK_SPACE, VK_NUM_LOCK, VK_DOWN, VK_DELETE,
     VK_POWER},
    {VKPAD_8, VKPAD_5, VKPAD_2, VKPAD_0, VKPAD_SLASH, VK_RIGHT, VK_INSERT,
     VK_SLEEP},
    {VKPAD_9, VKPAD_6, VKPAD_3, VKPAD_PERIOD, VKPAD_ASTERIX, VKPAD_MINUS,
...
#else
static const unsigned char kb_map_num[ARRAY_SIZE(scan_pins)]
                                     [ARRAY_SIZE(drive_pins)] = KB_MAP_NUM;
#endif

#ifndef KB_MAP_FN
static const unsigned char kb_map_fn[ARRAY_SIZE(scan_pins)][ARRAY_SIZE(
    drive_pins)] = {
    {VK_PAUSE, VK_POWER, VK_EURO, VK_SLEEP, VK_RCTRL, VK_WAKEUP, VK_CTRL,
     VK_F5},
    {VK_Q, VK_TAB, VK_A, VK_ESC, VK_Z, VK_NCHG, VK_TILDE, VK_1},
...
#else
static const unsigned char kb_map_fn[ARRAY_SIZE(scan_pins)]
                                    [ARRAY_SIZE(drive_pins)] = KB_MAP_FN;
#endif
...
void kb_rmv_ghost_key(u32 *pressed_matrix) {
  ...
  foreach_arr(i, drive_pins) {
    for (int j = (i + 1); j < ARRAY_SIZE(drive_pins); ++j) {
      ...
      // four or three key at "#" is pressed at the same time, should remove
      // ghost key
      if (mix &&
          (!BIT_IS_POW2(mix) || (pressed_matrix[i] ^ pressed_matrix[j]))) {
        // remove ghost keys
...
#endif
    if (filt_en) {
      // mtrx_cur[i] = (mtrx_last[i] ^ mtrx_tmp) ^ (mtrx_last[i] | mtrx_tmp);
      // //key_matrix_pressed is valid when current and last value is the same
      mtrx_cur[i] = (~mtrx_last[i] & (mtrx_pre[i] & mtrx_tmp)) |
...
static inline void kb_remap_key_row(int drv_ind, u32 m, int key_max,
                                    kb_data_t *kb_data) {
  foreach_arr(i, scan_pins) {
    if (m & 0x01) {
      ...
      // lock_button_pressed = 1;
        ...
        kb_data->keycode[kb_data->cnt++] =
            (VK_ZOOM_IN == kc) ? VK_EQUAL : VK_MINUS;
...
static inline void kb_remap_key_code(u32 *pressed_matrix, int key_max,
                                     kb_data_t *kb_data, int numlock_status) {
...
u32 kb_scan_key_value(int numlock_status, int read_key, unsigned char *gpio) {
  ...
  u32 key_changed = key_debounce_filter(
      pressed_matrix, (numlock_status & KB_NUMLOCK_STATUS_POWERON) ? 0 : 1);
  ...
  if (key_changed) {
    ...
    if (repeat_key.key_change_flg == KEY_SAME && repeat_key.key_repeat_flg &&
        clock_time_exceed(repeat_key.key_change_tick,
                          (KB_REPEAT_KEY_INTERVAL_MS - 5) * 1000)) {
      repeat_key.key_change_tick = clock_time();
      key_changed = 1;
    }
  }
