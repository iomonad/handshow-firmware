...
typedef struct {
  ...
  //	u32 gid0;		//pipe0 code,	used as sync code for control
  //pipe in hamster
...
typedef struct {
  ...
  //	u32 gid;		//pipe0 code,	used as sync code for control
  //pipe in hamster
  ...
  u8 seq_no;
  u8 pno;
  u8 data[MOUSE_FRAME_DATA_NUM *
          sizeof(mouse_data_t)]; // now the data length is variable, if the
                                 // previous no ACK, data will send again in
                                 // next time
...
typedef struct {
  ...
  u8 data[4 * sizeof(kb_data_t)]; // now the data length is variable, if the
                                  // previous no ACK, data will send again in
                                  // next time
